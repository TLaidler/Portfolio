...
Podemos tentar demonstrar que Cristiano Ronaldo é um grande jogador de futebol ao descrever cada passe em cada jogo da Liga dos Campeões da UEFA que ele jogou (dados brutos muito dificeis de se analisar, tendo em vista que Cristiano Ronaldo jogou por aproximadamente vinte anos), ou podemos calcular que ao final da temporada de 2015, Ronaldo teve uma média de passes na carreira de 0,313. Essa é uma estatística descritiva.

Acontece que a **média** de passes é uma simplificação grosseira das várias temporadas de Ronaldo. Os especialistas em futebol possuem um bando de estatísticas descritivas que consideram mais valiosas do que a média de passes. Ainda assim, idealmente, gostaríamos de achar o equivalente econômico da média de passes, ou algo ainda melhor, para explicar a saúde econômica de uma parcela considerável do país. Queremos uma _medida simples e precisa_ de como o bem-estar econômico do trabalhador típico tem mudado nos últimos anos. Uma tentativa razoável seria calcular a mudança na renda per capita no decorrer de uma geração, (aproximadamente trinta anos). A renda per capita é uma média simples: renda total dividida pelo tamanho da população. Por essa medida, a renda per capita brasileira passou de algo em torno de US$ 3 mil em 1990 para a faixa de US$ 7–9 mil em 2020 (o valor exato depende da fonte — Banco Mundial, FMI, IBGE — e da metodologia — nominal, ajustada por inflação ou PPP).

    Há um problema. O cálculo está tecnicamente correto, mas totalmente errado em termos da nossa interpretação. Além das complicações de correção pela inflação (um dólar em 1990 vale aproximadamente o dobro de um dólar em 2020), a renda média do Brasil não é igual a renda do brasileiro médio!
...
--- MD CELL 9 ---
A *variância* — quadrado do desvio padrão — é a forma matematicamente mais conveniente de medir essa dispersão:

$\sigma^{2} = \text{Var}(X) = \sum_{i=1}^{N} \frac{(x_{i} - \bar{x})^2}{N}$

E o **desvio padrão** propriamente dito é a raiz quadrada disso: $\sigma = \sqrt{\text{Var}(X)}$, que tem a vantagem de estar na mesma unidade dos dados originais.

> Por que dividir por $N$ e não por $N-1$? Boa pergunta — vamos resolver isso com cuidado mais adiante (na seção sobre regressão), quando ficar claro que existe uma distinção entre o desvio padrão **populacional** (divide por $N$) e o **amostral** (divide por $n-1$). Por ora, fique com a intuição.

> **Notação:** vou usar $\bar{x}$ para média amostral, $\mu$ para média populacional, $\sigma^2$ para variância e $\sigma$ para desvio padrão. Em algumas referências aparece $\tilde{x}$, mas reservarei esse símbolo para a *mediana*, conforme convenção mais comum.
--- MD CELL 10 ---
...
Já discutimos a média (1° momento) e a variância (2° momento central). Faltam dois personagens importantes: o terceiro e o quarto momentos centrais — **assimetria** (*skewness*) e **curtose** (*kurtosis*) — que descrevem, respectivamente, o quanto a distribuição é torta para um lado e o quanto ela tem caudas pesadas.

A **assimetria** é definida (na sua forma padronizada) como:

$\text{Skew} = E\!\left[\left(\frac{X - \mu}{\sigma}\right)^3\right]$

Quando ela é zero, a distribuição é simétrica (caso da Normal). Quando positiva, a cauda direita é mais longa — pense na distribuição de renda, em que poucos super-ricos puxam a cauda longe da mediana. Quando negativa, a cauda esquerda domina.

A **curtose** mede o quanto há "massa nas caudas" em relação ao corpo da distribuição:

$\text{Kurt} = E\!\left[\left(\frac{X - \mu}{\sigma}\right)^4\right]$

A Normal tem curtose 3 (por isso é costume reportar o *excesso de curtose*, $\text{Kurt} - 3$). Distribuições com curtose maior que 3 são chamadas **leptocúrticas**: têm pico mais alto e caudas mais grossas — eventos extremos são menos raros do que uma Normal sugeriria. Isso é absolutamente central em finanças: retornos diários de ações apresentam curtose tipicamente entre 5 e 10, e essa "gordura na cauda" é, em boa parte, o que faz crises financeiras serem subestimadas por modelos que assumem normalidade (lembre-se do que falamos sobre os modelos de risco em 2008).

Distribuições com curtose menor que 3 são **platicúrticas**: caudas finas, eventos extremos raros — o exemplo canônico é a distribuição uniforme.
--- MD CELL 22 ---
Perceba o que ocorre com a média, mediana e moda da distribuição normal dependendo da sua assimetria: numa distribuição com assimetria positiva, a média é puxada para a direita pelos valores extremos, ficando *à direita* da mediana, que por sua vez fica à direita da moda. Numa assimetria negativa, a ordem se inverte. Numa distribuição perfeitamente simétrica, as três coincidem. É uma forma rápida de "diagnosticar" assimetria sem nem calcular o terceiro momento: basta comparar média e mediana.
--- MD CELL 24 ---
# 3)
...
Pela definição de probabilidade condicional temos a chamada **regra do produto**:

$P(A \text{ e } B) = P(A|B)\times P(B) = P(B|A)\times P(A)$

Igualando as duas expressões à direita e dividindo por $P(B)$, chegamos à forma clássica do **Teorema de Bayes**:

$P(A|B) = \frac{P(B|A) \times P(A)}{P(B)}$

> Atenção a uma distinção importante: a *definição de probabilidade condicional* (a regra do produto acima) é uma coisa. O *Teorema de Bayes* (a inversão dos condicionamentos) é outra. O segundo é uma consequência elementar do primeiro, mas são objetos conceitualmente distintos — Bayes é um *método de atualização de crenças* a partir de evidência, e essa é a fonte do seu poder.
--- MD CELL 36 ---
...
O número buscado no casamento, portanto, seria o décimo primeiro número da linha 100 (escolher 10 de 100). De fato, quando algum convidado inevitavelmente se queixar de seu lugar na mesa, poderemos lembrá-lo do tempo que seria necessário para considerar todas as possibilidades: são aproximadamente $1{,}73 \times 10^{13}$ combinações possíveis (cerca de 17 trilhões). Presumindo que passássemos apenas um segundo avaliando cada uma, precisaríamos de mais de **meio milhão de anos** (são pouco mais de 31 milhões de segundos em um ano).
--- MD CELL 47 ---
...
--- MD CELL 50 ---
 Não é difícil perceber que há um problema quando se trata de trabalhar com tal tipo de variável. Apesar de entendermos que a variável aleatória serve apenas para mapear o espaço amostral (e, portanto, nos permite trabalhar com probabilidades de forma mais simples), tais definições ainda são muito abstratas. Perceba que quando falamos de uma variável aleatória do tipo "vida útil de uma lâmpada", não se envolve uma fórmula direta que nos permita calculá-la. Ou seja, não podemos performar uma análise quantitativa sobre isso. Por isso, trabalhar com espaços é útil, por exemplo, podemos ir atrás da probabilidade de uma lâmpada durar até 1 ano, e isso é possível obter modelando os dados.

> **Atenção fundamental: probabilidade e densidade de probabilidade são coisas diferentes!**
>
> Para uma variável **discreta**, faz sentido perguntar "*qual a probabilidade de $X = 5$?*". A resposta é um número entre 0 e 1, e a soma dessas probabilidades sobre todos os valores possíveis de $X$ dá exatamente 1. A função que associa cada valor a sua probabilidade é a **função massa de probabilidade** (PMF), $P(X=x)$.
>
> Para uma variável **contínua**, essa pergunta não faz sentido — a probabilidade de $X$ assumir *exatamente* um valor real específico é sempre zero. (Pense bem: existem infinitos valores reais entre 1,70 m e 1,71 m de altura. A probabilidade de alguém medir exatamente $1{,}703827\ldots$ m é zero — sempre haverá algum dígito a mais.) O objeto que faz sentido é a **função densidade de probabilidade** (PDF), $f(x)$, e probabilidades só emergem quando *integramos* essa densidade em um intervalo:
>
> $P(a \leq X \leq b) = \int_a^b f(x)\,dx$
>
> A densidade $f(x)$ pode ser maior que 1 em valores específicos — não há nada que a impeça disso. O que precisa valer é $\int_{-\infty}^{\infty} f(x)\,dx = 1$. Confundir densidade com probabilidade é um erro recorrente, principalmente quando se lê um valor alto de $f(x)$ num gráfico e se interpreta como "probabilidade alta de $X = x$" — não é.
--- MD CELL 51 ---
...
$CV = \dfrac{DP}{E(x)} = \dfrac{\sqrt{p(1-p)}}{p} = \sqrt{\dfrac{1-p}{p}}$;
...
Esse modelo nasceu em 1837, quando o matemático francês Siméon Denis Poisson publicou um livro sobre julgamentos criminais. Ele estava tentando entender uma pergunta aparentemente prosaica: dado que eventos raros acontecem aleatoriamente ao longo do tempo — condenações injustas, mortes por acidente, decaimentos radioativos que ainda nem haviam sido descobertos — qual a probabilidade de observarmos exatamente $k$ deles num dado intervalo?

O resultado dele ficou esquecido por décadas. Foi resgatado em 1898 pelo estatístico russo-alemão Ladislaus Bortkiewicz, que aplicou a fórmula a um dos conjuntos de dados mais bizarros já estudados: o número de soldados prussianos mortos por coices de cavalo por ano, por corpo de exército, ao longo de 20 anos. A distribuição de Poisson previu os dados com precisão assustadora. Desde então, ela aparece em todo lugar — de chamadas telefônicas a mutações genéticas, de partículas radioativas a goleiros levando frango.

> Esquece a fórmula por um segundo. A distribuição de Poisson responde a uma pergunta muito específica:
>
> *Se eventos acontecem de forma aleatória e independente ao longo do tempo (ou do espaço), a uma taxa média conhecida, quantos eventos vou observar num intervalo fixo?*

Note os três ingredientes:

1. **Eventos discretos** — você conta eventos inteiros (0, 1, 2, 3, ...). Não faz sentido "2,3 chamadas telefônicas".
2. **Independência** — um evento ocorrer não afeta a probabilidade do próximo. Ligações no *call center* não conspiram entre si.
3. **Taxa constante** — em média, a frequência é estável no intervalo considerado. Chamamos isso de $\lambda$ (lambda), a taxa.

> Se esses três ingredientes valem, Poisson é a *única* resposta possível. Não é uma escolha de modelagem — é uma consequência matemática inevitável. É como o Teorema Central do Limite para soma de variáveis: dadas as condições, o resultado emerge.

##### A fórmula

Se $X$ é o número de eventos num intervalo, com taxa média $\lambda$, então:
...
> Repare numa coisa estranhíssima (e... bonita, vai): a **média é igual à variância**. Isso é uma *assinatura única* da Poisson. Se você medir um processo no mundo real e encontrar média $\approx$ variância, é um sinal forte de que Poisson está em jogo. Se a variância é muito maior que a média, você tem **overdispersion** — sintoma de que algum dos três ingredientes acima foi violado (tipicamente a independência ou a taxa constante), e nesse caso modelos como a binomial negativa costumam encaixar melhor.

##### De onde ela vem? (a derivação física)

Imagine um intervalo de tempo $[0, T]$. Divida-o em $N$ subintervalos minúsculos de tamanho $T/N$. Em cada subintervalo, ou acontece um evento (com probabilidade $p$) ou não — exatamente o experimento de Bernoulli que vimos antes. Então o número total de eventos no intervalo é uma **Binomial**$(N, p)$, com taxa esperada $\lambda = Np$.

A sacada é fazer $N \to \infty$ e $p \to 0$ *mantendo* $\lambda = Np$ fixo. O limite da Binomial, nessa configuração, é exatamente a Poisson:

$$\lim_{N \to \infty} \binom{N}{k} p^{k}(1-p)^{N-k} = \dfrac{\lambda^{k}\, e^{-\lambda}}{k!}$$

A seguir, vamos fazer essa conta passo a passo. Manterei a notação do material — $N$ para o número de tentativas, $n$ para o número de sucessos observados ($k$ na fórmula acima) e $P$ para a probabilidade individual:

Se $N \longrightarrow \infty$ e $P \cdot N \longrightarrow \lambda$ tal que $\lambda$ é uma constante, podemos manipular o P da binomial rapidamente, multiplicando tudo por (N/N) tal que:

$P = \frac{N!}{n!(N-n)!} * p^n (\frac{N}{N})^n *(1-\frac{N}{N}p)^{N-n}$

Escrevendo deste modo, podemos ler $P*N$ como nossa média constante. Não só isso, podemos simplificar a combinação da seguinte maneira (note o cuidado com a expansão do fatorial — o produto vai de $N$ até $N-n+1$, e o que sobra é $(N-n)!$, que cancela com o denominador):

$\dfrac{N!}{n!(N-n)!} = \dfrac{N(N-1)(N-2)\cdots(N-n+1)\cdot(N-n)!}{n!\,(N-n)!} = \dfrac{N(N-1)\cdots(N-n+1)}{n!}$
...
> É por isso que Poisson é chamada de "**lei dos eventos raros**". Ela é o limite de muitas tentativas independentes, cada uma com probabilidade minúscula de sucesso, com taxa total estável — o lugar onde o "discreto" e o "raro" se encontram. E é também por isso que a aparição da Poisson em domínios tão diversos (decaimento radioativo, chamadas telefônicas, mutações por base de DNA por replicação, coices de cavalo no exército prussiano) não é coincidência: todos esses processos satisfazem aproximadamente os três ingredientes que listamos no início — discretos, independentes, taxa estável — e, portanto, *têm* que cair na mesma distribuição.
--- MD CELL 71 ---
...
> **Enunciado formal (para registro):** Sejam $X_1, X_2, \ldots, X_n$ variáveis aleatórias **independentes e identicamente distribuídas** (i.i.d.), com média $\mu$ e variância $\sigma^2$ *finita*. Então, conforme $n \to \infty$:
>
> $\sqrt{n}\,\dfrac{\bar{X}_n - \mu}{\sigma} \xrightarrow{d} \mathcal{N}(0,\,1)$
>
> Ou seja, a média amostral $\bar{X}_n$ se distribui aproximadamente como uma Normal de média $\mu$ e variância $\sigma^2/n$ — independentemente da forma da distribuição original (desde que a variância seja finita).
>
> Três pontos que costumam passar batido e merecem atenção:
>
> 1. **Variância finita é hipótese necessária.** Existem distribuições com cauda tão pesada que a variância diverge — o exemplo canônico é a distribuição de Cauchy. Para essas, o TCL **não vale**: por mais que você faça médias de mais e mais amostras, a média amostral *não* converge para uma Normal.
> 2. **i.i.d. é hipótese forte.** Em dados reais com correlação temporal (séries financeiras, por exemplo), o TCL clássico falha — existem versões para sequências dependentes, mas com cuidados extras.
> 3. **Velocidade de convergência depende da assimetria.** Distribuições muito assimétricas exigem $n$ muito maior para que a aproximação Normal seja boa (o teorema de Berry-Esseen dá um limite quantitativo para isso).
--- MD CELL 88 ---
...
Para além da definição de probabilidade, é importante abordarmos o conceito de Verossimilhança (*likelihood*). O termo pode soar estranho (pouco comum), talvez fosse mais didático chamarmos de "plausibilidade". Para entendermos do que se trata, imaginemos o mesmo caso da moeda, explorada anteriormente. No entanto, dessa vez, ao invés de assumirmos sua honestidade ($p=0,5$) para depois inferirmos seu resultado, vamos inverter a ordem. Iremos partir de um resultado *a priori*, para questionar a honestidade da moeda (talvez a probabilidade de dar cara seja maior que coroa).

Ou seja:

- **Pergunta 1 (Probabilidade):** "Eu sei que a moeda é honesta ($p=0.5$). Qual a chance de sair 7 caras em 10 lançamentos?"

→ Aqui você fixa o modelo ($p=0.5$) e pergunta sobre os dados que ainda não viu.

- **Pergunta 2 (Likelihood):** "Eu já joguei a moeda 10 vezes e saíram 7 caras. Quão plausível é que $p=0.5$? E se fosse $p=0.7$? E $p=0.9$?"

→ Aqui você fixa os dados (já observou 7 caras) e pergunta sobre qual modelo os gerou.

O termo foi cunhado por Ronald Fisher em 1921. Ele percebeu algo profundo: quando temos dados em mãos e queremos inferir o que os causou, não estamos mais no domínio da probabilidade no sentido clássico — porque os dados já aconteceram, sua "probabilidade" agora é 1. O que varia é nossa hipótese sobre o mundo.

Fisher escreveu que precisávamos de uma palavra nova justamente porque "probability" carregava bagagem demais. Ele queria deixar claro que *likelihood* não é uma distribuição de probabilidade sobre as hipóteses — se você somar (ou integrar) a *likelihood* sobre todos os valores possíveis de $p$, o resultado não dá 1. Ela não é uma probabilidade.

A fórmula é numericamente idêntica:

$$
L(\theta \mid x) = f(x \mid \theta)
$$

Mas o objeto matemático é diferente — uma é função de $x$, a outra é função de $\theta$.

Exemplo concreto (sua moeda, 7 caras em 10):

$$
L(p \mid \text{7 caras}) = \binom{10}{7} p^7 (1-p)^3
$$

- $L(0.5) \approx 0.117$
- $L(0.7) \approx 0.267$ ← máximo
- $L(0.9) \approx 0.057$

Esses números não somam 1 se você variar $p$ de 0 a 1. Eles dizem: "dado que vi 7 caras, $p=0.7$ é cerca de 2,3× mais compatível com os dados que $p=0.5$". É uma medida de compatibilidade relativa, não uma probabilidade. Nesse caso, poderíamos questionar o dono da moeda (ora, apesar de ser possível tirarmos 7 caras em 10 lançamentos em uma moeda honesta, é razoavelmente improvável).

É exatamente isso que a Máxima Verossimilhança (MLE) faz: encontra o $\theta$ que maximiza $L$ — o parâmetro que torna os dados observados mais "esperáveis" (quão mais improvável?).

---

#### Axiomas de Kolmogorov

O nome pode assustar, mas os axiomas de Kolmogorov nada mais são do que os princípios básicos que adotamos como verdadeiros para que os cálculos de probabilidade que vimos até aqui funcionem. Se você conseguiu acompanhar as anotações até aqui, achará um tanto intuitivo. Veja:
...
$$
P(E)\in \mathbb {R}, \quad P(E)\geq 0 \qquad \forall E\in {\mathcal {F}}
$$
...
Este é o pressuposto de $\sigma$-aditividade:

Qualquer sequência contável de conjuntos disjuntos (sinônimo de eventos mutuamente exclusivos)
...
(Perceba que se trata da regra do "e" que exploramos no início das anotações).

---

Para mostrar como esses princípios e cálculos que vimos até aqui funcionam e são coerentes, irei apresentar um dos argumentos mais elegantes da filosofia da probabilidade: *Dutch Book Arguments*. Ele responde a uma pergunta aparentemente abstrata ("por que devemos seguir os axiomas de Kolmogorov?") com uma resposta brutalmente prática: porque senão você perde dinheiro, garantido.

Imagine um cassino, mas ao contrário. Você é a casa. Você fixa os preços das apostas, e qualquer pessoa pode escolher de que lado da aposta quer estar — comprar ou vender. Seus "preços" refletem o quanto você acredita em cada evento.

Um "*Dutch book*" — o nome vem do jargão de apostadores britânicos do início do século XX, possivelmente uma referência pejorativa à habilidade comercial holandesa — é um conjunto de apostas que, somadas, garantem prejuízo para um dos lados, independentemente do que aconteça no mundo. Não é azar. É aritmética.

O argumento, formalizado por Frank Ramsey em 1926 e independentemente por Bruno de Finetti em 1931, diz o seguinte: se suas crenças sobre o mundo não obedecem aos axiomas da probabilidade, então existe um conjunto de apostas — todas que você mesmo consideraria justas — que te faz perder dinheiro com certeza. E o inverso também vale: se suas crenças obedecem aos axiomas, nenhum *Dutch book* pode ser montado contra você.

Vamos fazer com números. Esquece filosofia por um minuto.

**Regra do jogo:** se você atribui probabilidade $p$ a um evento $E$, isso significa que você está disposto a:

- Pagar $p$ por um bilhete que paga $1$ se $E$ ocorre (e $0$ caso contrário).
- Vender esse mesmo bilhete por $p$.

Você é indiferente entre os dois lados — esse é o seu "preço justo".

---

##### Exemplo 1: probabilidades que não somam 1

Suponha que amanhã ou chove ou não chove (eventos mutuamente exclusivos e exaustivos). Você diz:

- $P(\text{chove}) = 0.7$
- $P(\text{não chove}) = 0.5$

"Parece inofensivo? Vou te quebrar."

Eu te vendo os dois bilhetes:

- Bilhete A (paga $1$ se chover): você me paga $0.70$
- Bilhete B (paga $1$ se não chover): você me paga $0.50$

Você me dá $1.20$ no total. Amanhã, necessariamente um dos dois bilhetes paga $1$ (e só um). Você recebe $1$.

**Resultado:** você perde $0.20$. Não importa se chove ou não. Garantido.

---

##### Exemplo 2: probabilidades que somam menos que 1
...
Agora eu inverto: compro os dois bilhetes de você, pagando $0.30 + 0.40 = 0.70$. Amanhã recebo $1$ de você (porque um dos eventos ocorre).

Você perde $0.30$ garantidos.

---

##### Exemplo 3: violação de monotonicidade

Suponha:

- $P(\text{sair rei de copas}) = 0.1$
- $P(\text{sair uma carta vermelha}) = 0.05$

Mas rei de copas implica carta vermelha! A probabilidade do evento mais específico não pode exceder a do mais geral. Aqui também dá pra montar um *Dutch book* — fica como exercício, mas a estrutura é a mesma.

---

A direção "se viola → *Dutch book* existe" é o que mostramos com exemplos. A direção "se satisfaz → nenhum *Dutch book* existe" requer um argumento de separação por hiperplanos (essencialmente, um teorema de dualidade em programação linear), mas a intuição é que as probabilidades coerentes formam um conjunto convexo que não pode ser "explorado" arbitrariamente (depois pesquise sobre o Teorema de Ramsey-de Finetti).

Existe também um argumento mais sofisticado, devido a David Lewis, que mostra que se você atualiza suas crenças por algo diferente da regra de Bayes ao receber nova informação, também pode ser vítima de um *Dutch book* — desta vez espalhado no tempo. Isso é frequentemente usado como justificação para o condicionamento bayesiano como única regra de atualização racional.

O que esse argumento faz é interessante: ele transforma uma questão normativa ("como devo organizar minhas crenças?") numa questão pragmática ("quais crenças não me arruínam?"). É uma das poucas vezes na filosofia em que conseguimos derivar "deve" de "é".

De Finetti foi mais longe: ele argumentou que probabilidade não existe como propriedade objetiva do mundo. Sua frase famosa — "A probabilidade não existe" — significa que probabilidades são graus de crença de agentes, e o que os disciplina não é correspondência com fatos físicos, mas coerência interna, garantida pelo *Dutch book*.

É a fundação filosófica do bayesianismo subjetivista. Você não precisa acreditar em "frequências verdadeiras" ou "propensões objetivas". Basta querer não ser passado para trás.

É importante destacar, no entanto, que em casos de probabilidades imprecisas, trabalhos modernos mostram que se você usa intervalos de probablidade em vez de valores pontuais, pode evitar *Dutch books* estritos enquanto permanece em estado de incerteza honesta sobre suas próprias crenças (Walley, 1991). O *Dutch book* é um teste de coerência, não de acurácia. Suas probabilidades podem ser perfeitamente coerentes e completamente erradas sobre o mundo. Coerência é necessária, não suficiente, para boa epistemologia.

No entanto, podemos afirmar que tudo que foi estudado até aqui tem peso intelectual sério, é coerente e eficaz para nos auxiliar a medir o que, intuitivamente, não seria possível ser medido: a incerteza.
--- MD CELL 99 ---
# 5)
...
> **OBS 2 — uma armadilha sutil: independência $\neq$ não-correlacionado.** Duas variáveis aleatórias são *independentes* quando o valor de uma não carrega *nenhuma* informação sobre o valor da outra ($P(X,Y) = P(X) \cdot P(Y)$). Duas variáveis têm *correlação zero* quando não existe relação **linear** entre elas — mas pode existir uma relação não-linear forte. O exemplo clássico: tome $X \sim \mathcal{N}(0,1)$ e $Y = X^2$. Aqui $\text{Cov}(X, Y) = 0$ (a correlação de Pearson é zero), mas $Y$ é uma função *determinística* de $X$ — ou seja, são tão dependentes quanto se pode ser. O coeficiente de Pearson não detectaria essa relação; coeficientes não-paramétricos como Spearman ou Kendall capturam relações monotônicas (cobrindo mais casos), mas mesmo eles falham com relações simétricas como a parábola. **Moral:** correlação $= 0$ não autoriza concluir independência. Sempre olhe o gráfico de dispersão antes de tirar conclusões.
--- MD CELL 108 ---
...
--- MD CELL 110 ---
Geralmente usamos o valor de 5% como limiar mais comum utilizado nas inferências estatísticas. Essa probabilidade é conhecida como *nível de significância* ($\alpha$) e representa o limite superior para a probabilidade de observação de algum padrão de dados *se a hipótese nula fosse verdadeira*. Ou seja, podemos rejeitar **H0** no nível $\alpha = 0{,}05$ se houver uma chance menor do que 5% de obter um resultado no mínimo tão extremo quanto o que observamos *caso* a hipótese nula fosse verdadeira.

> **Atenção semântica fundamental:** $\alpha$ **não** é a probabilidade de **H0** ser verdadeira dados os nossos dados. Isso seria $P(H_0 \mid \text{dados})$ — uma quantidade *a posteriori* bayesiana que só pode ser calculada com um *prior* explícito. O que $\alpha$ controla é $P(\text{dados ao menos tão extremos} \mid H_0)$ — na direção oposta da implicação. Trocar uma pela outra é provavelmente o erro mais difundido em estatística aplicada, e foi tão crônico nas ciências empíricas que a *American Statistical Association* publicou em 2016 um *statement* inteiro dedicado a corrigi-lo. Vou repetir e sublinhar: **p-valor não é a probabilidade de H0 ser verdadeira.**

##### Erros tipo I, tipo II e poder

Um teste de hipóteses pode errar de duas formas distintas:

| | **H0 é verdadeira** | **H0 é falsa** |
|---|---|---|
| **Rejeitamos H0** | Erro tipo I (probabilidade $\alpha$) | Acerto |
| **Não rejeitamos H0** | Acerto | Erro tipo II (probabilidade $\beta$) |

O nível $\alpha$ (geralmente 5%) é o quanto estamos dispostos a errar na direção de rejeitar H0 quando ela era verdadeira — um *falso positivo*. Já $\beta$ é a probabilidade de **não** rejeitar H0 quando ela era falsa — um *falso negativo*. A quantidade $1 - \beta$ é o que chamamos de **poder do teste**: a probabilidade de detectar um efeito que realmente existe.

Há uma assimetria proposital aqui: $\alpha$ costuma ser fixado pelo pesquisador (5%, 1%, etc.) *antes* da análise. $\beta$ depende do tamanho da amostra $n$, da magnitude real do efeito (que tipicamente não conhecemos a priori) e da variabilidade dos dados. Amostras pequenas levam a poder baixo — e poder baixo significa que "não rejeitar" pode estar simplesmente refletindo nossa cegueira, não a ausência de efeito.

> **Implicação prática:** "falhei em rejeitar H0" **não é** "provei H0". É "com os dados que tenho, não consegui descartá-la". A diferença não é semântica — é o que separa um cientista cauteloso de um que comete erros tipo II sistematicamente.
--- MD CELL 111 ---
...
Definimos $\alpha = 5\%$ *previamente* à coleta dos dados, para que essa decisão não fique contaminada pelo que observarmos (esse é o ponto de evitar *p-hacking*). Esse $\alpha$ controla a taxa de erro tipo I: rejeitar **H0** incorretamente.

Se a estatística observada não cai na região crítica, *falhamos em rejeitar* **H0** — e essa formulação importa: não estamos *provando* **H0**, apenas dizendo que os dados disponíveis não foram suficientes para descartá-la no nível $\alpha$ escolhido. Pode ser que **H0** seja falsa e o nosso teste simplesmente tenha pouco poder.

--- MD CELL 112 ---
O que chamamos de *p-value* é a probabilidade específica de obter um resultado no mínimo tão extremo quanto o observado caso **H0** fosse verdadeira. Repetindo, no risco de soar pedante: é $P(\text{dados extremos} \mid H_0)$, não $P(H_0 \mid \text{dados})$.

##### Intervalos de Confiança — o irmão pragmático do teste de hipótese

Antes de continuarmos com os testes, vale apresentar uma ferramenta intimamente relacionada e, em muitos casos, mais informativa: o **intervalo de confiança** (IC). Em vez de respondermos "rejeito H0 ou não?" — uma decisão binária —, o intervalo de confiança nos entrega um *range* plausível para o parâmetro que estamos estimando.

A forma mais comum, para a média de uma variável normal com variância conhecida, é:

$IC_{95\%}(\mu) = \bar{X} \pm 1{,}96 \cdot \dfrac{\sigma}{\sqrt{n}}$

(O 1,96 vem do fato de que 95% da massa da Normal padrão fica entre $-1{,}96$ e $+1{,}96$.) Para amostras pequenas e $\sigma$ desconhecido, troca-se a Normal pela t de Student e o $\sigma$ pelo desvio amostral $s$.

> **Cuidado interpretativo (de novo).** A frase correta é "*construímos um procedimento tal que, se repetíssemos o experimento muitas vezes, 95% dos intervalos construídos dessa forma conteriam o parâmetro verdadeiro*". A frase **errada** (e tentadora) é "*há 95% de chance do parâmetro estar neste intervalo*" — essa interpretação só seria válida em estatística bayesiana, com um *prior* explícito (e nesse caso o objeto se chamaria *intervalo de credibilidade*, não de confiança). A diferença é sutil mas importante: o parâmetro é fixo (embora desconhecido); o intervalo é a variável aleatória.

**Por que isso costuma ser mais informativo que um teste?** Porque um IC carrega simultaneamente: (i) o ponto estimado, (ii) a precisão da estimativa (largura do intervalo) e (iii) o teste de hipóteses correspondente — se o valor sob H0 (digamos, zero) não está dentro do IC de 95%, então rejeitamos H0 ao nível 5%. Você ganha três coisas pelo preço de uma.
...
--- MD CELL 134 ---
Os dados coletados são compatíveis com **H0** ao nível $\alpha = 5\%$. Ou seja, partindo que **H0** é verdadeiro (as estrelas observadas fazem parte da mesma distribuição), nossos dados concordam com a maioria das amostras que viriam dessa mesma distribuição. Portanto, **falhamos em rejeitar H0** no nível adotado.
> *Cuidado semântico (estamos de pleno acordo aqui com tudo que discutimos antes):* isso **não** significa "95% de chance dos dados virem da mesma distribuição". Significa apenas que, com o teste e o tamanho de amostra que temos, não conseguimos descartar a hipótese de que vêm. Ausência de evidência contra **H0** não é evidência a favor de **H0** — pode ser apenas falta de poder estatístico (amostra pequena, ruído alto, ou efeito real pequeno). Para *afirmar* que os dados vêm da mesma distribuição precisaríamos de outro arcabouço (testes de equivalência, ou uma análise bayesiana com prior explícito).
--- MD CELL 135 ---
...
> **Aviso importante: correlação $\neq$ causalidade.** O estudo Whitehall é justamente um exemplo *observacional* — não experimental. Por mais sofisticado que seja o modelo, ele só consegue medir *associações* (ajustadas por covariáveis observadas). Causalidade exige algo mais: ou um experimento randomizado, ou suposições explícitas sobre o mecanismo gerador dos dados (variáveis instrumentais, regressão descontínua, *difference-in-differences*, modelos causais à la Pearl). Sem isso, três armadilhas espreitam qualquer regressão:
>
> 1. **Confundidores não-observados** — alguma variável que afeta tanto a explicativa quanto a resposta, e que você não mediu. Exemplo: nível de educação afeta tanto tipo de emprego quanto saúde, e se você não controla por isso, o "efeito do controle no emprego" capta também o efeito da educação.
> 2. **Causalidade reversa** — talvez não seja o trabalho de baixo controle que cause estresse, mas pessoas estressadas (por outros motivos) que acabem em trabalhos de baixo controle.
> 3. **Viés de seleção** — quem entra na amostra não é representativo da população alvo.
>
> Pearl, em *The Book of Why* (2018), e Imbens & Rubin, em *Causal Inference for Statistics, Social and Biomedical Sciences* (2015), são duas referências essenciais para quem quer ir além de "encontrei um $p < 0{,}05$, logo X causa Y".
--- MD CELL 139 ---
...
> **Pressupostos do MQO (Gauss-Markov), que costumam ser deixados na entrelinha:** o estimador de MQO é o "melhor" (de menor variância dentre os lineares e não-viesados) apenas se uma série de condições for satisfeita. São elas:
>
> 1. **Linearidade** — a relação entre $x$ e $y$ é, de fato, linear nos parâmetros. Se for, por exemplo, exponencial, o ajuste linear será viesado por construção.
> 2. **Exogeneidade** — $E[\epsilon \mid X] = 0$. Os resíduos não devem carregar informação sobre $X$. Se carregarem (caso típico de variável omitida), o estimador é viesado.
> 3. **Homocedasticidade** — a variância dos resíduos é constante ao longo de $X$. Se a variância cresce com $X$ (heterocedasticidade), o estimador continua não-viesado, mas a inferência sobre seus erros-padrão fica errada.
> 4. **Independência dos resíduos** — sem autocorrelação. Particularmente problemático em séries temporais.
> 5. **Em regressão múltipla: não-multicolinearidade severa** — variáveis explicativas não podem ser combinações lineares quase-perfeitas umas das outras (senão a matriz de design fica quase singular e os coeficientes ficam instáveis).
>
> Quando se acrescenta a hipótese de que $\epsilon \sim \mathcal{N}(0, \sigma^2)$, ganhamos as distribuições nominais dos coeficientes (t de Student) — necessárias para construir IC e testes. O cuidado a desenvolver é o de sempre olhar o **gráfico de resíduos** (resíduos vs. $\hat{y}$, e Q-Q plot) antes de confiar em um $p$-valor de coeficiente. Eu não conheço nenhum estatístico sério que pule essa etapa.
--- MD CELL 148 ---
...
--- MD CELL 194 ---
Perceba que estamos sorteando numeros aleatórios que estão 'presos' em dado limite (nos casos, cada distribuição segue uma função diferente). A partir desses dados, podemos comparar o que esperamos de uma distribuição, por exemplo, exponencial e comparar com nossos dados (digamos que temos algo que se assemelhe com uma exponencial, gostaríamos de saber o quão diferente ela está de dados aleatórios que sigam a mesma distribuição).

Um bom exemplo do quanto este método pode ser útil é a **agulha de Buffon**, um método desenvolvido no século XVIII para o cálculo do *pi* a partir de números aleatórios. O método de Monte Carlo torna o exercício trivial — e vale a pena fazer o "hello world" do método antes de irmos para o exemplo financeiro:

> **Estimando $\pi$ por Monte Carlo (versão geométrica, mais simples que a agulha original):** Inscreva um círculo de raio 1 num quadrado de lado 2 (vai do $-1$ ao $+1$ nos dois eixos). A área do círculo é $\pi$; a área do quadrado é 4. Logo, a razão entre as áreas é $\pi / 4$.
>
> Sorteie $N$ pontos uniformemente dentro do quadrado. Conte quantos caem dentro do círculo (aqueles com $x^2 + y^2 \leq 1$). A fração de pontos dentro do círculo, para $N$ grande, converge para $\pi/4$ — então $\hat{\pi} = 4 \cdot N_{\text{dentro}} / N_{\text{total}}$.
>
> Esse é o esqueleto de qualquer aplicação de Monte Carlo: substituir uma quantidade analítica difícil (uma integral, uma esperança, um valor em risco) por uma média sobre amostras aleatórias. O erro decresce como $1/\sqrt{N}$ — devagar, mas independente da dimensão do problema, o que é a grande sacada do método (e o que o torna competitivo em integrais multidimensionais que métodos determinísticos não conseguem atacar).
--- MD CELL 195 ---
