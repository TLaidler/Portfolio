...
## 🧬 Exemplo integrador — a altura média dos brasileiros

> **Use este exemplo único para amarrar TODA a Aula 1.** Cada conceito aparece marcado em **negrito** no momento em que entra. É o "fio condutor" para a Luiza pendurar a teoria.

### O problema e a verdade que não conhecemos

Queremos a **altura média de todos os brasileiros** — uma **população** de $N \approx 200$ milhões. Não temos como medir os 200 milhões, então vamos **inferir a partir de uma amostra**.

Suponha que a verdade (que nunca veremos diretamente) seja:
$$\mu = 1{,}70\text{ m}, \qquad \sigma = 0{,}10\text{ m}\ \ (\sigma^2 = 0{,}01\text{ m}^2).$$

Aqui já entram quatro ideias: $\mu$ é a **média** (1º momento), $\sigma^2$ a **variância** (dispersão na unidade ao quadrado), $\sigma$ o **desvio padrão** (dispersão na unidade original, em metros), e — crucial — **$\mu$ e $\sigma$ são PARÂMETROS**: números *fixos* da população, escritos com letra grega, que não conhecemos. As alturas humanas seguem aproximadamente uma **distribuição Normal** (simétrica, formato de sino), descrita 100% por $(\mu,\sigma)$ — então ~68% dos brasileiros medem entre 1,60 e 1,80 m, e ~95% entre 1,50 e 1,90 m.

### Sutileza nº 1: amostra ruim — o pecado do viés (acurácia)

Imagine medir $n=50$ pessoas... colhidas **numa quadra de basquete**. A maioria serão jogadores, e a média sobe para, digamos, $\bar x = 1{,}75$ m, com dispersão também inflada ($s = 0{,}25$ m, pois misturamos gente altíssima com algumas pessoas normais).

Esse erro é de **acurácia**, não de azar: é um **viés de seleção**, um erro *sistemático* — a "mira torta" do alvo de dardos. E aqui mora a lição mais importante:

> **Viés NÃO some com mais dados.** Se eu medir 50 mil jogadores em vez de 50, a média não converge para 1,70 — converge para a altura média *dos jogadores*, ~1,90 m. Aumentar $n$ só conserta o **tremor**, nunca a **mira torta**. Por isso a amostra precisa ser, antes de tudo, **aleatória**.

### Sutileza nº 2: amostra boa — estimador, estimativa e o n−1

Agora pegamos uma amostra **aleatória** de $n=200$ pessoas de todo o país. Calculamos:
$$\bar X = \frac{1}{n}\sum_{i=1}^{n} X_i \qquad\text{(a média amostral)}.$$

Repare na distinção fina: $\bar X$ é o **estimador** — uma *receita* aplicada a dados aleatórios, e portanto ela própria uma **variável aleatória** (outra amostra → outro valor). Quando a calculamos e sai o número $\bar x = 1{,}71$ m, isso é a **estimativa** (a receita vs. o prato pronto). Para a dispersão usamos
$$s^2 = \frac{1}{n-1}\sum (X_i - \bar X)^2,$$
com o **$n-1$** (correção de Bessel) justamente para não subestimar $\sigma^2$ — e obtemos $s \approx 0{,}10$ m.

Note que $\bar X$ é um **estimador não-viesado**: $E[\bar X] = \mu$. Em média, acerta o alvo. O viés é zero — toda a "imperfeição" dele será **tremor**, não "mira torta".

### Sutileza nº 3: quão confiável é o 1,71? (precisão e erro padrão)

A estimativa 1,71 ≠ 1,70 verdadeiro. Isso é **erro amostral** normal — a pergunta é: *de que tamanho é esse tremor esperado?* A resposta é o **erro padrão da média**:
$$\mathrm{EP}(\bar X) = \frac{\sigma}{\sqrt n} = \frac{0{,}10}{\sqrt{200}} \approx 0{,}0071\text{ m} \approx 0{,}7\text{ cm}.$$

Ou seja, a média de uma amostra aleatória de 200 pessoas tipicamente cai a meros ~0,7 cm da verdade. E aparece a **tirania do √n**: para reduzir esse erro pela metade (para 0,35 cm), eu preciso de **4×** mais gente ($n = 800$), não o dobro.

> **Precisão × acurácia, lado a lado:** a amostra de basquete poderia ter $n$ gigante e um erro padrão minúsculo — seria **precisa**, mas continuaria centrada em 1,90: **inacurada**. Precisão não compra acurácia. Tiros agrupadíssimos, mas no canto errado do alvo.

### Sutileza nº 4: por que isso tudo funciona? (LGN + TCL + convergência)

Duas garantias teóricas sustentam o método:

- **Lei dos Grandes Números:** conforme $n$ cresce, $\bar X_n \to \mu$. Mais precisamente, $\bar X_n \xrightarrow{P} \mu$ — **convergência em probabilidade** (a Lei *Fraca*). O motor é $\mathrm{Var}(\bar X_n) = \sigma^2/n \to 0$: o tremor encolhe até sumir.
- **Teorema do Limite Central:** a distribuição de $\bar X$ é aproximadamente $\mathcal N\!\big(\mu,\ \sigma^2/n\big)$ — **mesmo que** a altura individual não fosse perfeitamente Normal. É **convergência em distribuição**, e é ela que nos autoriza a usar o "1,96" para fazer afirmações probabilísticas sobre a estimativa.

### Sutileza nº 5: juntando tudo no EQM, e a ponte para o intervalo

Para a amostra **boa**, o erro quadrático médio é puro tremor:
$$\mathrm{EQM}(\bar X) = \mathrm{Var}(\bar X) + \underbrace{\mathrm{Vi\acute es}^2}_{=\,0} = \frac{\sigma^2}{n} = \frac{0{,}01}{200} = 0{,}00005\text{ m}^2.$$
Para a amostra de **basquete**, o termo de viés ($\approx 0{,}20$ m)² domina tudo e **não cai com $n$** — é o que a torna irrecuperável.

Por fim, em vez de só cuspir um número (**estimação pontual**, $\bar x = 1{,}71$), entregamos uma faixa (**estimação intervalar**):
$$\bar x \pm 1{,}96\,\frac{\sigma}{\sqrt n} = 1{,}71 \pm 1{,}96(0{,}0071) \approx 1{,}71 \pm 0{,}014 = [\,1{,}696;\ 1{,}724\,]\text{ m},$$
que de fato contém a verdade 1,70. *(O cuidado interpretativo correto do IC fica para a Aula 2.)*

> **Caixa de diagnóstico — "e se a amostra *aleatória* desse 1,75 ± 0,25?"** Aí teríamos uma pista valiosa. Com $\sigma = 0{,}10$ e $n = 200$, o erro padrão é 0,7 cm; uma média de 1,75 estaria a $\frac{1{,}75 - 1{,}70}{0{,}0071} \approx 7$ **erros-padrão** da verdade — algo astronomicamente improvável numa amostra honesta. Somado a isso, $s = 0{,}25 \gg \sigma = 0{,}10$ (dispersão grande demais). Os dois sinais juntos gritam: **essa amostra não é aleatória — está contaminada** (provavelmente com os jogadores da Sutileza nº 1). A magnitude da discrepância, medida em erros-padrão, é o próprio detector de viés. *(Esse raciocínio é exatamente a semente do teste de hipóteses da Aula 2.)*

### ✅ Conceitos costurados neste único exemplo

População × amostra · parâmetro ($\mu,\sigma$) × estimador ($\bar X$) × estimativa (1,71) · média · variância · desvio padrão · distribuição Normal · amostragem aleatória · **viés de seleção / acurácia** · **precisão** · correção $n-1$ · estimador não-viesado · erro padrão $\sigma/\sqrt n$ · tirania do √n · **EQM = Var + Viés²** · LGN · convergência em probabilidade · TCL · convergência em distribuição · estimação pontual × intervalar.
