...
"""
=============================================================================
T8 Full Pipeline Null Model — ret_20_savgol Variant
=============================================================================

Same as t8_full_pipeline.py, but REPLACES sg_velocity_51 with ret_20_savgol
(the strong artifact that was removed from the pipeline).

Question: if sg_velocity_51 (weak artifact) produces GENUINE in combination,
does ret_20_savgol (strong artifact, p=0.80 individually) do even better?
...
Output: relatorios/t8_full_pipeline_ret20.md + relatorios/pngs/t8fp_ret20_*.png
"""
...
# Same features as real model, but REPLACING sg_velocity_51 with ret_20_savgol
SELECTED_FEATURES = [
    "ret_20_savgol", "tstat_50", "volatility_20", "volatility_50",
...
def compute_features(bars, config, verbose=False):
    """Compute all features using FeatureRegistry + manual ret_20_savgol."""
    import io
    ...
    # Manually compute ret_20_savgol (removed from FeatureRegistry)
    close_raw = df_feat["close"].values.astype(np.float64)
    sg = savgol_causal(close_raw, SG_WINDOW, SG_POLY)
    ret_20 = pd.Series(sg).pct_change(20).values
    df_feat["ret_20_savgol"] = ret_20
    if "ret_20_savgol" not in all_feature_names:
        all_feature_names.append("ret_20_savgol")
...
def main():
    print("=" * 70)
    print("  T8 FULL PIPELINE NULL MODEL — ret_20_savgol Variant")
    print("=" * 70)
    t_start = time.time()
    ...
    plt.tight_layout()
    path1 = os.path.join(PNG_DIR, "t8fp_ret20_sr_distributions.png")
    ...
    plt.tight_layout()
    path2 = os.path.join(PNG_DIR, "t8fp_ret20_active_analysis.png")
    ...
    plt.tight_layout()
    path3 = os.path.join(PNG_DIR, "t8fp_ret20_boxplot.png")
    plt.savefig(path3, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Plot: {path3}")

    # ── Report ─────────────────────────────────────────────────────────
    report = f"""# T8 Full Pipeline Null Model — ret_20_savgol Variant

**Data:** 2026-03-26
**Objetivo:** Testar se o pipeline completo com **ret_20_savgol** (artefato forte,
p=0.80 no T8 individual) substituindo sg_velocity_51 gera alpha genuino.

**Contexto:** O T8 full pipeline com sg_velocity_51 resultou em **GENUINE** (p=0.00).
ret_20_savgol foi a feature dominante dos modelos Mod1-Mod5 (MDA=0.177) mas foi
descartada por ser artefato individual. Agora testamos se, em combinacao com features
genuinas (tstat_50 etc.), o artefato forte tambem produz alpha genuino no pipeline.
...
![SR Distributions](pngs/t8fp_ret20_sr_distributions.png)

![Active Trades Analysis](pngs/t8fp_ret20_active_analysis.png)

![Box Plot Comparison](pngs/t8fp_ret20_boxplot.png)
...
"""
    ...
    report_path = os.path.join(REPORT_DIR, "t8_full_pipeline_ret20.md")
