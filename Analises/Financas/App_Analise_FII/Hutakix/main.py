...
def encontrar_arquivo_download(nome_arquivo):
    ...
    arquivo_procurado = os.path.join(pasta_download, nome_arquivo)
    if os.path.isfile(arquivo_procurado):
...
df_ifix.rename(columns = {'4. close':'IFIX.SA'}, inplace=True)
...
time.sleep(4) #esperar um tempinho para completar o download
driver.quit()
...
Filtro_DY = tabela_dwnl[(tabela_dwnl.DY >= minimo_DY)*(tabela_dwnl.DY < maximo_DY)] 
...
data_final = pd.Timestamp.today()
data_inicial = data_final - pd.DateOffset(months=108) #pegar dados desde 9 anos atrás
...
#Tornar possivel escolher e-mails que receberão a análise
string = input("Digite o e-mail para receber a análise (para mais de um, separe com virgula): ")
remetentes = list(string.split(","))
