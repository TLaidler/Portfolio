...
Ambos os objetivos foram atingidos. O banco de dados relacional em SQLite armazena as curvas de luz \textbf{originais} obtidas das fontes externas (catálogo \texttt{B/occ/asteroid} do VizieR e banco do Grupo do Rio), totalizando 802 curvas positivas e 3 negativas reais. A partir dessas curvas, o \textit{dataset} de treino/teste é montado em memória pela \textit{pipeline}, incorporando 186 negativos artificiais (recortados manualmente de segmentos sem ocultação das próprias positivas) e 702 curvas sintéticas negativas geradas pelo simulador físico de \cite{Simulator}. O conjunto final usado nos experimentos contém, portanto, 1693 amostras. A \textit{pipeline} de classificação foi implementada em Python e está documentada para reprodução.
...
\textbf{Etapa 3 -- Construção do conjunto de dados.} As curvas armazenadas no banco são lidas e o \textit{dataset} de treino/teste é montado \textbf{em memória}, fora do banco SQL: segmentos sem ocultação são recortados manualmente de curvas positivas (com revisão humana e exclusão da positiva original do conjunto de treino para evitar vazamento); em seguida, curvas sintéticas negativas geradas pelo simulador físico de \cite{Simulator} são incorporadas. O \textit{dataset} final reúne 1693 amostras (Tabela~\ref{tab:dataset_fonte}).

\begin{table}[!h]
\centering
\caption{Composição do \textit{dataset} utilizado nos experimentos. Apenas as linhas marcadas com $\dagger$ correspondem a curvas fisicamente armazenadas no banco SQL; as demais são geradas pela \textit{pipeline} em memória, no momento da montagem do conjunto de treino/teste.}
\label{tab:dataset_fonte}
\begin{tabular}{lc}
\toprule
\textbf{Fonte} & \textbf{Quantidade} \\
\midrule
Curvas positivas do banco (VizieR + Grupo do Rio)$^\dagger$ & 802 \\
Curvas negativas do banco$^\dagger$ & 3 \\
...
\textbf{Desbalanceamento dos dados disponíveis.} A principal dificuldade enfrentada no período foi o forte desbalanceamento das fontes de curvas reais: o catálogo \texttt{B/occ/asteroid} do VizieR e o banco do Grupo do Rio reúnem majoritariamente eventos positivos (curvas com ocultação detectada), com apenas 3 negativas reais disponíveis. Esse desequilíbrio inviabilizaria o treinamento direto dos modelos, pois um classificador poderia atingir alta acurácia simplesmente predizendo sempre a classe majoritária. A solução adotada foi uma estratégia combinada de \textit{data augmentation}: (i)~recorte manual de segmentos sem ocultação a partir das próprias curvas positivas (com revisão humana e exclusão da positiva original do conjunto de treino, evitando vazamento), gerando 186 negativos artificiais; e (ii)~incorporação de 702 curvas sintéticas negativas geradas pelo simulador físico de \cite{Simulator}, que reproduz ruído fotométrico, cintilação atmosférica e geometria observacional. O Experimento 7, no qual o teste é restrito a curvas exclusivamente reais, mostra queda apenas marginal de desempenho (F1 de 0,9906 para 0,9821), sugerindo que o simulador produz curvas suficientemente fiéis para treinamento. Ainda assim, a validação em um cenário puramente observacional, com negativas reais em volume comparável às positivas, permanece como limitação reconhecida.
