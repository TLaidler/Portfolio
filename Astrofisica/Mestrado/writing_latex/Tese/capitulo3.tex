...
\textbf{Explorar relações de causa e efeito:} é possível ajustar um modelo em dados sem o efeito de interesse; e depois, coletar dados sob o efeito. O modelo irá prever as novas observações (sem efeito). Nesse caso, o erro de predição poderá ser atribuído ao efeito, sugerindo hipóteses causais. \textcolor{red}{fiquei confuso aqui.}

\textbf{Redução e simplificação:} em dados multidimensionais, técnicas de redução de dimensionalidade agrupam observações semelhantes e revelam estrutura em dimensão menor, ganhando clareza interpretativa. \textcolor{red}{isto lembra um pouco decomposição em componentes principais.}
...
\textcolor{red}{(a frase parece iniciar-se pela equação acima, o que fica bem estranho no texto.)}

\st{Onde} \textcolor{red}{onde} p é a probabilidade prevista pelo modelo e a perda (loss) deverá ser aplicada a um único ponto. Ou seja, para um conjunto de \(m\) exemplos, com rótulos \(y^{(i)} \in \{0,1\}\) e probabilidades preditas \(\hat{p}^{(i)}\), a função é
\begin{equation}
...
O hiperparâmetro \(\alpha\) (ou \(C = 1/\alpha\)) controla a força da penalização: \(\alpha\) grande significa uma crença prévia forte de que os pesos devem ser pequenos; \(\alpha\) pequeno deixa o modelo mais livre. A escolha de \(\alpha\) é feita por \textbf{validação cruzada} (Seção~\ref{sec:validacao_modelos}): treina-se o modelo com diferentes valores de \(\alpha\) e seleciona-se aquele que produz o melhor desempenho médio nas dobras de validação \citep{geron2019maos}. \textcolor{red}{Há vários termos ao longo do texto que não são comuns em astronomia (eu, ao menos, não conheço). Por exemplo, "dobras de validação". Veja o quanto vale a pena dar alguma noção do que se trata.}
...
A Regressão Logística serve de base conceitual para entender classificadores probabilísticos e foi também treinada e avaliada na \textit{pipeline} desta dissertação, junto com os \hl{ensembles} \textcolor{red}{(procure optar pelas correspondentes palavras em português. Como para o \textit{ensembles}, outros casos ocorrem no texto.)} de árvores (Random Forest, XGBoost, CatBoost), que tipicamente oferecem melhor desempenho em dados tabulares e fornecem importância de variáveis. A função sigmoide \(\sigma(z) = 1/(1+e^{-z})\) está ilustrada na Figura~\ref{fig:sigmoide}.
...
Random Forest é um método de \textbf{ensemble}: em vez de uma única árvore, treina-se um grande número de árvores de decisão (por exemplo, centenas), cada uma em uma \textbf{amostra aleatória} dos dados (amostragem com reposição, ou \textit{bootstrap}) e usando, em cada divisão de nó, apenas um subconjunto aleatório das características \citep{geron2019maos, Levada2026}. A predição final é obtida por \textbf{votação majoritária}: cada árvore "vota" em uma classe e escolhe-se a classe que recebeu mais votos. A força da floresta está na \textbf{descorrelação} entre as árvores: o \hl{bagging} \textcolor{red}{(o que é bagging?)} e a escolha aleatória de atributos em cada nó garantem que as árvores errem em casos diferentes; ao agregar centenas de árvores, o modelo mantém o baixo viés das árvores individuais \hl{mas} \textcolor{red}{("mas"~ou "e"?)} reduz drasticamente a \textbf{variância}. Uma vantagem colateral é a estimativa \textbf{out-of-bag} (OOB): como cada árvore é treinada em aproximadamente 63\% dos dados (amostragem com reposição), cerca de 37\% das instâncias ficam "fora da bolsa"~para cada árvore e podem ser usadas como conjunto de validação interno, permitindo estimar o erro de generalização sem um conjunto de teste separado \citep{geron2019maos}. Uma variante que aumenta ainda a aleatoriedade são as \textbf{Extra-Trees} (árvores extremamente aleatórias), que utilizam limiares aleatórios para cada atributo nas divisões, podendo reduzir mais a variância e acelerar o treinamento \citep{geron2019maos}. \textcolor{red}{esta última frase não está clara. Ela é necessária? Podes oferecer uma descrição melhor?}
