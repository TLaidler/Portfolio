...
O hiperparâmetro \(\alpha\) (ou \(C = 1/\alpha\)) controla a força da penalização: \(\alpha\) grande significa uma crença prévia forte de que os pesos devem ser pequenos; \(\alpha\) pequeno deixa o modelo mais livre. A escolha de \(\alpha\) é feita por \textbf{validação cruzada} (Seção~\ref{sec:validacao_modelos}): treina-se o modelo com diferentes valores de \(\alpha\) e seleciona-se aquele que produz o melhor desempenho médio nas \textbf{dobras} (\textit{folds}) de validação \citep{geron2019maos}. Uma \emph{dobra} é cada uma das partições em que o conjunto de dados é repetidamente dividido na validação cruzada: em cada rodada, parte das dobras é usada para treinar e a dobra restante para avaliar, alternando-se até que todas tenham servido como avaliação. Esse procedimento é detalhado na Seção~\ref{sec:validacao_modelos}.
