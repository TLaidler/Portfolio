...
Os parâmetros são estimados pelo método dos mínimos quadrados (que também pode ser interpretado como nosso primeiro exemplo de algoritmo de \textit{otimização}). No geral, em notação vetorial, o modelo prediz $\hat{\vec{y}} = X\vec{\theta}$ e a função de custo é
\[
J(\vec{\theta}) = \sum_{i=1}^{m} \big( y^{(i)} - \vec{\theta}^\top \vec{x}^{(i)} \big)^2,
\]
cujo minimizador admite solução fechada; a \textbf{equação normal}: $\vec{\theta}^* = (X^\top X)^{-1} X^\top \vec{y}$ \citep{geron2019maos}.
Em geral, após estimar os parâmetros e construir
a reta de regressão, utilizamos essa equação recém descoberta para estimar valores de $y$ a partir de novos valores de $x$.

A equação de custo em notação vetorial é muito útil para generalizar soluções de problemas que possuem múltiplas variáveis independentes. Nesses casos, o grau de ajuste do hiperplano (ao invés de uma simples reta) aos vetores é definido em termos dos erros entre os verdadeiros valores de $y_i$ e suas predições $\hat{y}_i$. Ou seja, um modelo linear dado por:

\[
y_i = \beta_0 + \beta_1 x_{i1} + ... + \beta_k x_ik + \epsilon_i = \vec{x}_i^T \vec{\beta} + \epsilon_i \longrightarrow \vec{y} = X \vec{\beta} + \vec{\epsilon}
\]

Em que $X$ é a matriz dos dados e $\vec{\beta}$ é o vetor de parâmetros que controlam o hiperplano ótimo. Após o cálculo do vetor de parãmetros, podemos estimar $\vec{y}$ para novos dados de entrada. Em termos matemáticos, deseja-se encontrar o vetor $\vec{\beta}$ que minimiza a soma dos resíduos ao quadrado:

\[
J(\vec{\beta}) = \sum_{i=1}^{n} \epsilon_i^2 = \vec{\epsilon}^T \vec{\epsilon} = (\vec{y} - \vec{\hat{y}})^T (\vec{y} - \vec{\hat{y}}) = (\vec{y} - X \vec{\beta})^T (\vec{y} - X \vec{\beta})
\]

Assim, a condição para minimizar o custo passa por:

\[
\frac{\partial J(\vec{\beta})}{\partial \vec{\beta}} = X^T (\vec{y} - X \vec{\beta}) = \vec{0}
\]

Essa resolução funciona para qualquer dimensão $\mathbb{R}^d$. Nos exemplos mais fundamentais ($\mathbb{R}^2$) a minimização de custo passa por

\[
J(\alpha,\beta) = \sum_{i=1}^{n} \epsilon_i^2 = \sum_{i=1}^{n} (y_i - \alpha \beta x_i)^2 \longrightarrow \frac{\partial J}{\partial \beta} = 0
\]
...
A Regressão Logística é um classificador probabilístico que utiliza a estrutura dos modelos lineares para prever categorias discretas \citep{geron2019maos}, ao transformar combinações lineares de características em probabilidades válidas. Esse tipo de modelo calcula uma soma ponderada das características de entrada, mas, em vez de gerar o resultado diretamente (como o modelo de Regressão Linear) gera a \textit{logística} (função sigmoide) desse resultado. 

Suponha um problema de classificação em que se deseja verificar se um tumor é binigno ou não a partir do tamanho do mesmo. A figura a seguir ilustra a aplicação da regressão linear nos dados.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Tese_old/pngs/regressao_linear_ex1.png}
    \label{fig:placeholder}
\end{figure}

Nesse caso teríamos infinitos valores para a variável de saída. Uma possível solução seria considerar um limiar simples na metade da reta. No entanto, suponha que no mesmo conjunto de dados tivéssemos um elemento \textit{outlier}. A figura a seguir ilusta a situação (nesse caso o limiar não funciona bem).

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Tese_old/pngs/regressao_linear_ex2.png}
    \caption{Exemplo utilizado por \citep{Levada2026}, capítulo 5.}
    \label{fig:placeholder}
\end{figure}

A reta de regressão se altera significativamente com a adição de novos pontos, de modo que é fácil concluir que haverá erro na classificação. Além disso, note que na regressão linear a saída pode assumir valores fora do intervalo desejado para uma medida de probabilidade (entre 0 e 1). 
...
\]
A decisão é: classe 1 se \(\hat{p} \geq 0{,}5\), caso contrário classe 0. Vale lembrar que o valor de 0,5 pode ser alterado a depender da calibração do modelo, como discutido no início do capítulo.
...
Para evitar sobreajuste, aplica-se a \textbf{regularização} \citep{geron2019maos, Levada2026}, que é o método de restringir o grau de liberdade do modelo. A regularização \(\ell_2\) (\textbf{Ridge}) adiciona ao custo o termo \(\frac{\alpha}{2} \sum_j \theta_j^2\), resultando em encolhimento (\textit{shrinkage}) dos coeficientes: os pesos são reduzidos em direção a zero, mas raramente se anulam, de modo que todos os atributos permanecem no modelo.

%%tornar mais didático o parágrafo de regularização
...
\end{equation}
A acurácia é intuitiva, mas pode ser enganosa em conjuntos desbalanceados: um classificador que prediz sempre a classe majoritária pode atingir acurácia alta sem detectar nenhum evento raro.
...
\textbf{Revocação} (\textit{Recall}, ou sensibilidade) --- entre os exemplos verdadeiramente positivos, qual fração foi corretamente detectada:
\begin{equation}
\text{Revocação} = \frac{TP}{TP + FN}.
\end{equation}

\textbf{F1-score} --- média harmônica de precisão e revocação, oferecendo um compromisso entre ambas:
\begin{equation}
F_1 = 2 \cdot \frac{\text{Precisão} \cdot \text{Revocação}}{\text{Precisão} + \text{Revocação}} = \frac{2\,TP}{2\,TP + FP + FN}.
\end{equation}

\textbf{F$_\beta$-score} --- generalização do F1-score que permite atribuir peso diferente à revocação:
\begin{equation}
F_\beta = (1 + \beta^2) \cdot \frac{\text{Precisão} \cdot \text{Revocação}}{\beta^2 \cdot \text{Precisão} + \text{Revocação}}.
\end{equation}
Para $\beta > 1$, o $F_\beta$ penaliza mais os falsos negativos (favorecendo a revocação); para $\beta < 1$, penaliza mais os falsos positivos. Em aplicações de triagem científica, como a detecção de ocultações estelares, $\beta = 2$ é uma escolha natural, pois o custo de perder um evento genuíno (FN) é maior que o de investigar um alarme falso (FP) \citep{Saito2015}.

\textbf{AUC-ROC} (\textit{Area Under the Receiver Operating Characteristic Curve}) --- mede a capacidade discriminativa do modelo ao longo de todos os limiares de decisão possíveis. A curva ROC plota a \textbf{taxa de verdadeiros positivos} (revocação) contra a \textbf{taxa de falsos positivos} ($FP/(FP+TN)$) para cada limiar $\tau$. A área sob essa curva (AUC) varia de 0,5 (classificador aleatório) a 1,0 (classificador perfeito). A AUC-ROC é útil por ser invariante ao limiar escolhido, mas pode ser otimista em conjuntos desbalanceados \citep{Saito2015}.

\textbf{Curva Precision-Recall} --- alternativa à curva ROC, recomendada quando a classe positiva é rara ou quando o custo dos erros é assimétrico \citep{Saito2015}. Plota a precisão contra a revocação para cada limiar~$\tau$. A área sob a curva PR (AUC-PR) é mais informativa que a AUC-ROC em cenários desbalanceados, pois não é inflada por verdadeiros negativos abundantes.

Nesta dissertação, os modelos são avaliados por acurácia, precisão, revocação, F1-score e AUC-ROC no conjunto de teste (Capítulo~5). Para a análise de ajuste de limiar, utilizam-se também o $F_2$-score e a curva precision-recall.
