...
Os parâmetros são estimados pelo método dos mínimos quadrados. Em notação vetorial, o modelo prediz $\hat{\vec{y}} = X\vec{\theta}$ e a função de custo é
\[
J(\vec{\theta}) = \sum_{i=1}^{m} \big( y^{(i)} - \vec{\theta}^\top \vec{x}^{(i)} \big)^2,
\]
cujo minimizador admite solução fechada --- a \textbf{equação normal}: $\vec{\theta}^* = (X^\top X)^{-1} X^\top \vec{y}$ \citep{geron2019maos}. Para classificação, não usamos regressão linear diretamente; a \textbf{Regressão Logística} mantém a combinação linear, mas mapeia a saída ao intervalo $(0,1)$ por uma função sigmoide e utiliza o custo de entropia cruzada, para o qual \textbf{não existe solução fechada} --- o treinamento é feito por algoritmos iterativos (Seção~\ref{sec:otimizacao}).
