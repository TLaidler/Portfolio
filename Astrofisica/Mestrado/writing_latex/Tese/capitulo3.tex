...
O Capítulo~2 mostrou que cada campanha de ocultação estelar produz centenas a milhares de curvas de luz, cada uma das quais pode ser resumida por um conjunto de números --- amplitude do fluxo, profundidade da queda, estatísticas de ruído, entre outros. A questão central é: \emph{dada essa descrição numérica de uma curva, é possível decidir automaticamente se ela contém ou não uma ocultação?} Esse é, por definição, um problema de \textbf{classificação binária}, e os métodos de \textit{Machine Learning} são as ferramentas naturais para resolvê-lo. Este capítulo apresenta os fundamentos teóricos necessários para compreender os modelos treinados e avaliados no Capítulo~4.
...
\caption{Aplicação da regressão linear a um problema de classificação binária: a reta produz saídas fora do intervalo $[0,1]$ \citep{Levada2026}.}
    \label{fig:reglin_classif_ex1}
\end{figure}

Nesse caso teríamos infinitos valores para a variável de saída. Uma possível solução seria considerar um limiar simples na metade da reta. No entanto, suponha que no mesmo conjunto de dados tivéssemos um elemento \textit{outlier}. A Figura~\ref{fig:reglin_classif_ex2} ilustra a situação: o limiar não funciona bem.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{pngs/regressao_linear_ex2.png}
    \caption{O mesmo problema com um \textit{outlier}: a reta de regressão se desloca e o limiar passa a classificar incorretamente \citep{Levada2026}.}
...
Para evitar sobreajuste, aplica-se a \textbf{regularização}: restringe-se o grau de liberdade do modelo adicionando um termo de penalização à função de custo \citep{geron2019maos, Levada2026}. As três formas principais são:

\begin{itemize}
	\item \textbf{Ridge (\(\ell_2\)):} adiciona ao custo o termo \(\frac{\alpha}{2} \sum_j \theta_j^2\). Os coeficientes são \textit{encolhidos} em direção a zero, mas raramente se anulam --- todos os atributos permanecem no modelo. Funciona bem com \textit{features} correlacionadas e possui solução em forma fechada.

	\item \textbf{Lasso (\(\ell_1\)):} adiciona \(\alpha \sum_j |\theta_j|\). Tende a \textbf{zerar} coeficientes de atributos irrelevantes, promovendo esparsidade e atuando como seleção automática de variáveis. Em contrapartida, pode comportar-se de forma errática com muitas variáveis correlacionadas (tendendo a escolher apenas uma) ou quando o número de variáveis supera o de instâncias. A função de custo não é diferenciável em \(\theta_j = 0\), exigindo subgradientes na otimização.
...
Do ponto de vista bayesiano (estimativa MAP), Ridge equivale a uma distribuição prévia Gaussiana e Lasso a uma distribuição de Laplace (com pico em zero) sobre os parâmetros. O hiperparâmetro \(\alpha\) (ou \(C\), inversamente relacionado) controla a força da penalização e pode ser escolhido via validação cruzada. É essencial \textbf{escalonar} as \textit{features} (e.g.\ StandardScaler) antes de aplicar Ridge ou Lasso, pois ambas as penalidades são sensíveis à escala dos dados. Nesta dissertação, utiliza-se Ridge ou Lasso na Regressão Logística, conforme documentado no Apêndice~\ref{ap:hiperparametros}.
...
\end{equation}
A acurácia é intuitiva, mas pode ser enganosa em conjuntos desbalanceados. \textbf{Exemplo numérico:} considere 100 curvas de luz, das quais 10 contêm ocultação (positivas) e 90 não (negativas). Um classificador que \textbf{sempre} prediz ``negativo'' acerta 90 das 100 curvas --- acurácia de 90\% --- mas não detecta \textit{nenhuma} ocultação ($\text{revocação} = 0$). Esse paradoxo evidencia que, em cenários desbalanceados, a acurácia sozinha é insuficiente para avaliar o modelo.
