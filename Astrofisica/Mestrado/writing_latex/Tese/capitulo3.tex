...
A solução é a \textbf{regularização}: impor um custo adicional aos pesos grandes, forçando o modelo a manter coeficientes moderados e a distribuir a ``explicação'' entre múltiplas features em vez de apostar tudo em uma \citep{geron2019maos, Levada2026}. As três formas principais são:
...
% TODO: Inserir diagrama de validação cruzada k-fold (k=5):
% cinco linhas horizontais, cada uma com 5 blocos; em cada linha,
% um bloco diferente é destacado como "validação" e os demais como "treino".
% \begin{figure}[htbp]
% \begin{center}
%   \includegraphics[width=0.75\textwidth]{pngs/diagrama_kfold.png}
% \caption{Esquema da validação cruzada com $k=5$: em cada rodada, uma fatia diferente
% (em destaque) é usada como conjunto de validação, e as demais como treino.}
% \label{fig:kfold}
% \end{center}
% \end{figure}
