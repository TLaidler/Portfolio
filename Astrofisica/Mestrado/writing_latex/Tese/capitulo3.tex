...
\textbf{Diagnóstico.} O sinal mais claro de sobreajuste é um erro muito baixo no treino e um erro alto no teste ou validação (não generalizou). Ao analisar as \textbf{curvas de aprendizado} (erro em função do tamanho do conjunto de treino), isso aparece como um \textbf{gap} entre a curva de treino e a de validação --- o erro de validação para de cair e pode até subir. Em caso de subajuste, as curvas de treino e validação ficam próximas e em um \textbf{platô alto} de erro. 

A \textbf{validação cruzada} (dividir os dados em \(k\) partes, treinar em \(k-1\) e avaliar na parte restante, rotacionando) permite obter uma estimativa robusta do erro de generalização (média e desvio padrão), facilitando perceber quando o modelo começou a sobreajustar \citep{geron2019maos}.
