...
O que podemos ver é que a reta de regressão se altera significativamente com a adição de novos pontos, de modo que é fácil concluir que haverá erro na classificação em um conjunto de dados muito extenso e rico em informações. Além disso, a regressão linear permite com que a saída possa assumir valores fora do intervalo desejado para uma medida de probabilidade (entre 0 e 1). 

Ou seja, a regressão logística funcionará como modelos lineares (saída contínua), mas adaptado como modelo de classificação probabilística: a combinação linear \(\vec{\theta}^\top \vec{x}\) é mapeada pela logística ao intervalo \((0,1)\), interpretado como a probabilidade da classe positiva \citep{Levada2026}. Em notação vetorial, seja \(\vec{\theta}\) o vetor de parâmetros  e \(\vec{x}\) o vetor de atributos; a probabilidade predita é
\[
