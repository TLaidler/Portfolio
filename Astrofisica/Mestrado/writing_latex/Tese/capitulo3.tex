...
Suponha um problema de classificação no qual se deseja verificar a probabilidade de um tumor ser binigno ou não a partir do tamanho do mesmo. A figura a seguir ilustra a aplicação da regressão linear nos dados.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{pngs/regressao_linear_ex1.png}
\caption{Aplicação da regressão linear ao problema de classificação binária do tumor: a reta produz saídas fora do intervalo $[0,1]$ \citep{Levada2026}. Nosso conjunto de dados mostra quatro amostras de tumores benignos e quatro amostras de tumores não benignos, claramente distinguiveis pelo tamanho. Nesse exemplo simples, o valor de 0,5 para o recorte da reta divide perfeitamente as duas classificações.}
...
\begin{figure}[htbp]
    ...
    \caption{O mesmo problema anterior com um \textit{outlier} (anomalia): a reta de regressão se desloca e o limiar 0,5 passa a classificar incorretamente \citep{Levada2026}.}
    \label{fig:reglin_classif_ex2}
\end{figure}

A reta de regressão se altera significativamente com a adição de novos pontos, de modo que é fácil concluir que haverá erro na classificação. Além disso, a regressão linear permite com que a saída possa assumir valores fora do intervalo desejado para uma medida de probabilidade (entre 0 e 1). 
...
Para evitar problemas de sobreajuste (quando o modelo 'decora' os dados de treino, mas não consegue replicar o aprendizado em dados de teste), aplica-se a \textbf{regularização}: processo em que restringe-se o grau de liberdade do modelo ao adicionar um termo de penalização à função de custo \citep{geron2019maos, Levada2026}. As três principais formas principais são:

\begin{itemize}
	\item \textbf{Ridge (\(\ell_2\)):} adiciona ao custo o termo \(\frac{\alpha}{2} \sum_j \theta_j^2\). Os coeficientes são \textit{encolhidos} em direção a zero, mas raramente se anulam - todos os atributos permanecem no modelo. Funciona bem com \textit{features} correlacionadas, reduzindo overfit (sobreajuste) sem descartar características.
...
Do ponto de vista bayesiano (Estimativa MAP - Maximum A Posteriori), Ridge equivale a uma distribuição prévia Gaussiana e Lasso a uma distribuição de Laplace (com pico em zero) sobre os coeficiêntes do modelo (figura \ref{fig:gaussLaplace}). O hiperparâmetro \(\alpha\) (ou \(C\), inversamente relacionado) controla a força da penalização e pode ser escolhido via validação cruzada. É essencial \textbf{escalonar} as \textit{features} (e.g.\ StandardScaler) antes de aplicar Ridge ou Lasso, pois ambas as penalidades são sensíveis à escala dos dados. Nesta dissertação, utiliza-se Ridge ou Lasso na Regressão Logística, conforme documentado no Apêndice~\ref{ap:hiperparametros}.
