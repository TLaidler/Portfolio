...
As curvas de luz são séries temporais (tempo \(\times\) fluxo). Os modelos de classificação utilizados neste trabalho (Regressão Logística, Random Forest, XGBoost, CatBoost) recebem \textbf{vetores numéricos}, não as séries brutas. Por isso, cada curva é convertida em um vetor de \textbf{características} (features) extraídas da série --- por exemplo, amplitude, mínimo do fluxo suavizado, distância entre centróides do K-means, etc. O resultado é uma tabela em que cada linha é uma curva e cada coluna é uma dessas grandezas. As características \textbf{não} foram padronizadas (subtrair média e dividir pelo desvio padrão) antes da entrada nos classificadores: os modelos baseados em árvores são insensíveis à escala; para a Regressão Logística, optou-se por usar a mesma tabela para todos os modelos, permitindo comparação direta. Como discutido no Capítulo 3, a regularização é sensível à escala; no Capítulo 5, se a Regressão Logística mostrar sensibilidade, a padronização apenas para esse modelo pode ser testada em trabalho futuro.
...
No dataset gerado pela \textit{pipeline}, essas grandezas aparecem em colunas cujos nomes no código possuem um prefixo nominal (por exemplo, \texttt{IOTA\_depth}, \texttt{IOTA\_duration\_s}); esse prefixo é apenas interno à implementação e não implica critério oficial de nenhuma instituição. No texto, tratamos todas elas como métricas de profundidade, duração e ajuste de modelos.
