...
Além das \textit{features} descritas até aqui, a \textit{pipeline} incorpora um conjunto de métricas inspiradas em critérios observacionais discutidos em fóruns entre astrônomos amadores para análise de curvas difíceis de ocultação. Não há documento oficial que defina essas grandezas como critério definitivo para classificar uma curva como positiva; no texto referimo-nos a elas como \textbf{métricas de profundidade, duração e ajuste de modelos}, sem uso de rótulos institucionais \textcolor{red}{("formais" ou "adotados na literatura"?)}. A implementação concreta segue o módulo de engenharia de características do projeto (arquivo \texttt{occ\_features.py}), cujas fórmulas são resumidas abaixo.

\textbf{Baseline.} O nível de fluxo fora da ocultação é estimado pela mediana da série: \(\mathrm{baseline} = \mathrm{mediana}(F)\). Em implementações alternativas pode-se usar a média dos pontos com fluxo acima da mediana. A mediana é robusta a quedas pontuais e a \textit{outliers} \textcolor{red}{(note que quedas pontuais podem ser chamadas de outliers)}.
...
\textbf{Desvio do baseline.} O desvio padrão \(\sigma_{\mathrm{baseline}}\) é calculado apenas sobre os pontos com fluxo \(\geq \mathrm{baseline}\) (fora do dip), caracterizando o ruído fotométrico do nível de continuum \textcolor{red}{(o que é "ruído fotométrico do nível de continuum"?)}. Se houver poucos pontos acima do baseline, usa-se o desvio padrão de toda a série.

\textbf{SNR do dip.} \textcolor{red}{(Aqui, a menos de alguma referência bibliográfica que eu desconheça e que justifique o que escrevestes, acho melhor você dizer algo como "neste trabalho, vamos definir a grandeza depth/(sigma baseline" como SNR do dip)} A razão sinal-ruído da queda é
\begin{equation}
...
O vetor final de \textit{features} inclui: amplitude e desvio padrão do fluxo; máximo e mínimo da média móvel e do Savitzky-Golay; max drawdown; distância entre centróides do K-means no fluxo; mínimos \(-\log_{10}(p)\) dos testes \(t\) e KS \textcolor{red}{(K-S)} entre quartis; as estatísticas das derivadas primeira e segunda listadas acima; e as métricas de profundidade, duração e ajuste de modelos (profundidade do dip, SNR do dip, duração em segundos do maior run abaixo do baseline, número de frames no maior run, desvio padrão do baseline, fluxo mínimo, razão fluxo mínimo/baseline, \(\chi^2\) do modelo constante, \(\chi^2\) do modelo square well, razão \(\chi^2_{\mathrm{const}}/\chi^2_{\mathrm{sw}}\)). Todas as colunas de \textit{features} são numéricas; metadados como identificador da curva e rótulo (positivo/negativo) são mantidos em colunas separadas e não são usados como entrada do classificador. O resultado da etapa de engenharia de características é um arquivo tabular (por exemplo, \texttt{dataset\_final.csv}) em que cada linha é uma curva e as colunas são as \textit{features} mais o rótulo e o identificador.
...
\textcolor{red}{Acho importante destacar, em algum ligar adequado no início da dissertação, o tópico que discutimos na reunião e que relembro agora.}
\textcolor{red}{Encontrarmos um evento de ocultação pelo corpo principal numa curva de luz é o mínimo que esperamos do código, algo que deve acontecer sem falhas. No entanto, importa-nos bastante sinais mais sutis de eventos. Por exemplo, cita-se a descoberta do anel de Quaoar (colocar referência do artigo do Bruno, na Nature), descoberto após uma revisita na análise dos dados pelo autor da descoberta. Um ponto bastante relevante deste trabalho, então, é a capacidade de identificarmos tais eventos em curvas de luz de ocultação, de forma a darmos a devida atenção a caracatrísticas relevantes desde o primeiro momento e, eventualmente, não perdermos a oportunidade de publicarmos uma nova descoberta. Tal capacidade é apresentada entre os resultados desta dissertação. Você pode ajustar o texto como preferir, mas penso que não devemos deixar de mencionar isso.}
