...
\textcolor{red}{É interessante mostrar uma curva positiva que não tenha sido detectada como tal por algum dos modelos}
...
\begin{table}[htbp]
\centering
\caption{Métricas de desempenho no conjunto de teste --- Experimento 1. Conjunto completo de 28 \textit{features}, \textit{split} 80\%/20\%.}
...
\begin{table}[htbp]
\centering
\caption{Métricas de desempenho no conjunto de teste --- Experimento 2. Subconjunto de 14 \textit{features}, \textit{split} 80\%/20\%.}
...
\begin{table}[htbp]
\centering
\caption{Métricas de desempenho no conjunto de teste --- Experimento 3. 13 \textit{features} (sem \texttt{Feature\_Savgol\_Min}), \textit{split} 80\%/20\%.}
...
\begin{table}[htbp]
\centering
\caption{Métricas de desempenho no conjunto de teste --- Experimento 4. 12 \textit{features} (sem \texttt{kmeans\_centroid\_dist}), \textit{split} 80\%/20\%.}
...
\begin{table}[htbp]
\centering
\caption{Métricas de desempenho no conjunto de teste --- Experimento 5. 11 \textit{features} (sem \texttt{Feature\_Savgol\_Min} e sem \texttt{kmeans\_centroid\_dist}), \textit{split} 80\%/20\%.}
...
\begin{itemize}
	\item \textbf{Falso negativo} (ocultação real classificada como negativa): a curva é descartada ou recebe baixa prioridade de análise. O evento científico é perdido, potencialmente de forma irreversível --- a ocultação não se repetirá nas mesmas condições \textcolor{red}{(de fato, o evento não se repetirá mas, desde que os dados não sejam jogados fora, a informação continua lá. Sugiro que a frase seja algo como: evento deixa de ser prioridade para análise mais detalhada, podendo demorar para ser "redescoberto", inclusive levando à perda de trabalho contendo possível resultado inédito pelo grupo.)}.
...
Todos os quatro classificadores utilizados neste trabalho produzem \textbf{probabilidades calibradas} via o método \texttt{predict\_proba} \textcolor{red}{(o que é isso? já foi definido anteriormente?)}. A classificação binária é obtida pela aplicação de um limiar $\tau$:
%
\[
\hat{y} =
\begin{cases}
...
\begin{table}[htbp]
\textcolor{red}{a tabela parece sair do limite da página - se precisar, deixe-a em landscape}
\centering
...
As Figuras~\ref{fig:precision_recall_curve} e \ref{fig:metrics_vs_threshold} ilustram as curvas precisão-sensibilidade e as métricas em função do limiar (Experimento~3).

\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=0.75\linewidth]{Tese/pngs/exp3/precision_recall_curve.png}
\caption{Curvas precisão-sensibilidade dos quatro modelos (Experimento 3). Cada ponto corresponde a um valor de limiar $\tau$. \textcolor{red}{precisas deixar mais claro a interpretação destes gráficos. Ainda, talvez o eixo x pudesse iniciar em 0.9}}
...
\caption{Precisão, sensibilidade, F1-score e F$_2$-score em função do limiar de decisão $\tau$ (Experimento 3).}
...
As três curvas estão originalmente em formato \texttt{.dat} do PRAIA, com colunas para tempo (segundos UTC), fluxo normalizado e diversos \hl{diagnósticos fotométricos} \textcolor{red}{(acho saudável dar uns 2 exemplos aqui)}. Os dados foram \textbf{gentilmente cedidos pelo autor principal} de~\citet{Quaoar2023}, no qual a redução fotométrica completa do evento e a caracterização dos dois anéis Q1R e Q2R são apresentadas. Nenhuma das três curvas\textcolor{red}{, como já mencionado no texto,} figura no banco SQLite usado para treinar os modelos --- não estão no catálogo \texttt{B/occ/asteroid} do VizieR nem no \textit{drive} do Grupo do Rio --- garantindo que o teste é, de fato, sobre dados externos à distribuição de treino.
...
\begin{enumerate}
	...
	\item \textbf{Inferência:} o modelo \textbf{CatBoost} treinado no Experimento~5 (11 \textit{features}, sem \texttt{Feature\_Savgol\_Min} e sem \texttt{kmeans\_centroid\_dist}) é carregado a partir do diretório \texttt{outputs/resultado5}; o \textit{imputer} é recuperado, com \textit{fallback} para mediana local caso ocorra incompatibilidade de versão do \texttt{scikit-learn}.
\end{enumerate}

A escolha do CatBoost-Exp.~5 é deliberada: trata-se da configuração mais enxuta da \textit{pipeline} (menor número de \textit{features}, sem o passo de K-Means), reforçando que o desempenho elevado não depende das variáveis identificadas como dispensáveis pelo estudo de ablação (Seção~\ref{sec:ablation_kmeans}).
...
\begin{table}[htbp]
\centering
\caption{Resultados do estudo de caso de Quaoar sobre a curva completa. Probabilidade $\hat{p}$ atribuída pelo CatBoost (modelo do Exp.~5), profundidade do \textit{dip}, SNR do \textit{dip} e duração do maior \textit{run} abaixo do \textit{baseline}.}
...
\textcolor{red}{Não deixe de colocar nesta seção a análise que discutimos em reunião, em particular, isolar os eventos do anel e verificar se o ML os detecta.}
...
\subsection{Aplicação a recortes da curva}

O dicionário \texttt{CUT\_RANGES} do \textit{script} permite reexecutar a inferência sobre janelas restritas a cada componente físico do evento. Essa funcionalidade tem dois usos complementares:

\begin{itemize}
	\item \textbf{Isolamento da assinatura do corpo:} restringindo cada curva a uma janela de poucos segundos centrada no \textit{dip} principal, verifica-se que o classificador continua atribuindo $\hat{p} > 0{,}99$ --- ou seja, o sinal do corpo, por si só, é suficiente para a detecção.
	\item \textbf{Exposição apenas das regiões adjacentes (anéis e \textit{baseline}):} cortando explicitamente fora do \textit{dip} principal e mantendo somente as regiões antes/depois, onde se concentram as estruturas secundárias associadas aos anéis, é possível avaliar se o classificador atual responde a esses eventos rasos. O modelo treinado para detectar a ocultação do corpo, naturalmente, não foi otimizado para essa classe de evento; o resultado serve, antes, como diagnóstico do que poderia ser explorado em trabalhos futuros --- via classificação multi-classe ou modelo dedicado a eventos rasos --- como discutido no Capítulo~6.
\end{itemize}

O recorte é uma operação de uma linha (\texttt{cut\_curve(time, flux, t\_ini, t\_fim)}) e pode ser feita interativamente, no Debug Console do VS Code, durante uma sessão de depuração. Isso fornece ao analista uma ferramenta prática para explorar diferentes hipóteses sobre cada porção da curva sem retreinar o modelo nem reescrever a \textit{pipeline}.

\subsection{Interpretação}

O estudo de caso reforça, na prática, três conclusões dos experimentos formais:

\begin{enumerate}
	\item \textbf{Generalização para dados externos:} mesmo treinado em curvas predominantemente sintéticas e oriundas dos catálogos VizieR/Grupo do Rio, o modelo do Experimento~5 classifica corretamente registros de uma campanha observacional distinta, em três bandas espectrais e dois telescópios diferentes.
...
Como limitação, vale registrar que se trata de um \textit{n} pequeno (três curvas, todas do mesmo evento) e que os modelos atuais não estão otimizados para detectar separadamente os eventos de anel; uma avaliação sistemática nesse regime --- inclusive incorporando dados de campanhas similares no banco de treino --- é uma das direções recomendadas no Capítulo~6.
...
\begin{minipage}[t]{0.48\textwidth}
	...
	\small (d) CatBoost
\end{minipage}\\[0.5em]
\caption{Importância de variáveis por modelo --- Experimento 1 (28 \textit{features}). (a) Regressão Logística, (b) Random Forest, (c) XGBoost, (d) CatBoost. \textcolor{red}{As legendas dessas figuras estão muito pequenas}}
...
Os resultados apresentados nas seções anteriores baseiam-se em um único \textit{split} treino/teste por experimento. Embora útil para comparações controladas, essa abordagem não permite estimar a \textbf{variabilidade} do desempenho em função da partição dos dados. Para obter uma estimativa mais robusta da capacidade de generalização, implementou-se a \textbf{validação cruzada estratificada com $k=5$ dobras}, respeitando o agrupamento por curva de luz (todos os segmentos de uma mesma curva permanecem na mesma dobra, evitando \textit{data leakage}). \textcolor{red}{Não entendo o que fizestes aqui. Deve estar muito claro para você, mas não para quem lê. Você, às vezes e do meu ponto de vista, rebusca na linguagem às custas de clareza do que foi feito.}

O procedimento é o seguinte: os dados são divididos em 5 partições estratificadas (preservando a proporção de classes em cada dobra); em cada rodada, 4 dobras são usadas para treino e 1 para teste, e os quatro modelos (RF, XGBoost, CatBoost, Regressão Logística) são treinados e avaliados independentemente. As métricas coletadas por dobra são: F1-score, precisão, sensibilidade e AUC-ROC. O resultado final é reportado como \textbf{média $\pm$ desvio padrão} sobre as 5 dobras. \textcolor{red}{Não estendi o que foi feito aqui. As classes não são apenas duas: positiva ou negativa?}
