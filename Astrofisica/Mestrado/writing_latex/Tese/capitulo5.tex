...
\begin{itemize}
	...
	\item \textbf{Experimento 3:} 11 \textit{features}, porém com \textbf{teste somente em curvas reais} (as sintéticas permanecem no treino). É o teste de \textbf{generalização} mais próximo do uso real.
\end{itemize}

Os demais três experimentos --- a redução intermediária de \textit{features} para 14 (Experimento 4) e 13 variáveis (Experimento 5) e a ablação isolada da \textit{feature} \texttt{kmeans\_centroid\_dist} (Experimento 6) --- conduzem às mesmas conclusões e estão documentados em detalhe no Apêndice~\ref{ap:experimentos}, que reúne ainda uma variante do Experimento~3 com divisão 65\%/35\%, usada como ablação do tamanho do conjunto de treino. Suas métricas são consolidadas, ao lado das dos três experimentos-destaque, na Tabela~\ref{tab:comparacao_f1} (Seção~\ref{sec:comparacao}).
...
% -----------------------------------------------------------------------------
\section{Redução do conjunto de \textit{features}}
...
\begin{table}[htbp]
\centering
\caption{Métricas de desempenho no conjunto de teste --- Experimento 3. Teste somente em curvas reais (11 \textit{features}, divisão 80\%/20\%; teste com 198 curvas: 160 positivas e 38 negativas).}
\label{tab:metricas_7}
\begin{tabular}{lccccc}
\hline
\textbf{Modelo} & \textbf{Acurácia} & \textbf{Precisão} & \textbf{Sensibilidade} & \textbf{F1-score} & \textbf{AUC-ROC} \\
\hline
XGBoost         & \textbf{0,9848} & 0,9937 & 0,9875 & \textbf{0,9906} & 0,9984 \\
Random Forest   & 0,9798 & 0,9937 & 0,9812 & 0,9874 & 0,9965 \\
CatBoost        & 0,9798 & 0,9937 & 0,9812 & 0,9874 & 0,9980 \\
Regressão Log.  & 0,9646 & 0,9935 & 0,9625 & 0,9778 & 0,9974 \\
\hline
\end{tabular}
\end{table}
...
\textbf{Modelo} & \textbf{Exp. 1} & \textbf{Exp. 2} & \textbf{Exp. 3} & \textbf{Exp. 4} & \textbf{Exp. 5} & \textbf{Exp. 6} \\
 ...
 & \scriptsize{80/20} & \scriptsize{80/20} & \scriptsize{80/20} & \scriptsize{80/20} & \scriptsize{80/20} & \scriptsize{80/20} \\
\hline
Random Forest   & \textbf{0,9937} & 0,9874 & 0,9874 & 0,9874 & \textbf{0,9905} & \textbf{0,9906} \\
XGBoost         & 0,9844 & \textbf{0,9906} & \textbf{0,9906} & 0,9875 & 0,9843 & \textbf{0,9906} \\
CatBoost        & 0,9906 & 0,9905 & 0,9874 & \textbf{0,9906} & 0,9873 & 0,9874 \\
Regressão Log.  & 0,9873 & 0,9841 & 0,9778 & 0,9873 & 0,9873 & 0,9873 \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
	...
	\item \textbf{Generalização em curvas reais.} O Experimento 3 (teste somente em curvas reais) preserva o melhor F1 do teste misto (XGBoost, 0,9906) e mantém AUC-ROC $\geq 0{,}9965$, indicando que as \textit{features} capturam sinal real e não apenas artefatos da distribuição sintética. A queda de $\approx$2~pp observada na variante 65/35 (Apêndice~\ref{ap:experimentos}) decorre, em parte, da redução do conjunto de treino.

	\item \textbf{AUC-ROC.} Em todos os experimentos com teste misto (todos exceto o Experimento 3), a AUC-ROC permanece $\geq 0{,}9995$; no Experimento 3, $\geq 0{,}9965$.
\end{enumerate}

A Figura~\ref{fig:ROC1} apresenta as curvas ROC da linha de base em dados mistos (Experimento~1), com separação quase perfeita entre as classes. As curvas ROC dos demais experimentos --- incluindo a variante 65/35 do Experimento~3 (Figura~\ref{fig:ROC7}) --- estão reunidas no Apêndice~\ref{ap:experimentos}.
...
% -----------------------------------------------------------------------------
\section{Curvas de aprendizado dos modelos de \textit{boosting}}
\label{sec:curvas_aprendizado_exp}

A Seção~\ref{sec:curvas_aprendizado} (Capítulo~3) apresentou as curvas de aprendizado como ferramenta gráfica para diagnosticar sobreajuste: acompanha-se o erro em treino e em validação em função do tamanho do conjunto de treino ou do número de épocas. Aplicamos aqui a segunda variante aos dois modelos de \textit{boosting} da \textit{pipeline}, registrando o \textit{logloss} de treino e de teste a cada iteração, na configuração enxuta do Experimento~2 (11 \textit{features}, divisão 80\%/20\% por curva). A Figura~\ref{fig:learning_curves_pipeline} apresenta os resultados.

\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=0.95\textwidth]{Tese/pngs/learning_curves_xgb_catboost.png}
\caption{Curvas de aprendizado de (a)~XGBoost e (b)~CatBoost na configuração do Experimento~2: \textit{logloss} de treino (azul) e de teste (vermelho) em função das iterações de \textit{boosting}.}
\label{fig:learning_curves_pipeline}
\end{center}
\end{figure}

Dois padrões merecem registro. Primeiro, a perda de teste acompanha a de treino do início ao fim e estabiliza por volta da iteração 50--60, sem em nenhum momento inverter a tendência e voltar a crescer. Essa inversão seria a assinatura clássica de sobreajuste em \textit{boosting}; sua ausência indica que as 100 iterações adotadas param antes da região de risco. Segundo, os \textit{gaps} finais entre teste e treino são pequenos: 0,0215 no XGBoost (treino 0,0052; teste 0,0267) e 0,0125 no CatBoost (treino 0,0192; teste 0,0317). O CatBoost encerra com o menor \textit{gap}, comportamento coerente com sua regularização estrutural (\textit{ordered boosting} e árvores \textit{oblivious}, Capítulo~3), enquanto o XGBoost desce a perda de treino mais fundo, o que ajuda a explicar suas probabilidades mais polarizadas no estudo de caso de Quaoar (Seção~\ref{sec:estudo_caso_quaoar}). Em conjunto com a ablação do tamanho do treino (Apêndice~\ref{ap:experimentos}), que cobre a outra dimensão da curva de aprendizado, esses resultados completam o diagnóstico gráfico de generalização introduzido no Capítulo~3.
