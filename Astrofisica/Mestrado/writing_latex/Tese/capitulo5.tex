...
As figuras análogas das configurações com conjuntos maiores de \textit{features} (Experimentos 4 e 5), que permitem acompanhar como a importância relativa das variáveis se redistribui à medida que o conjunto é reduzido, estão reunidas no Apêndice~\ref{ap:experimentos}. Em particular, a alta importância atribuída a \texttt{kmeans\_centroid\_dist} naquelas configurações --- e o fato de sua remoção não degradar o desempenho --- é analisada no estudo de ablação (Seção~\ref{sec:resultados_exp4}).
