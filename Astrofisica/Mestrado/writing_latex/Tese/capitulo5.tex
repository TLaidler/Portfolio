...
Ao contrário dos modelos baseados em árvores, esses valores \ia{\textbf{não são normalizados para somar 1 ou 100}}, e por isso a escala do gráfico pode variar de 0 a valores como 3 ou 4, dependendo do problema.
...
O melhor desempenho em F1-score e acurácia foi obtido pelo \textbf{Random Forest} (F1 = 0,9937; acurácia = 0,9941). A sensibilidade de 98,75\% indica que a grande maioria das curvas com ocultação foi corretamente identificada; a precisão de 100\% indica ausência de falsos positivos nesse modelo. A Regressão Logística também alcança precisão de 100\%. \ia{A AUC-ROC muito alta ($\geq 0{,}9997$) confirma excelente capacidade de separação das classes.}
...
Partindo das 28 \textit{features} do Experimento~1, o conjunto de variáveis foi progressivamente reduzido, removendo aquelas identificadas como redundantes ou pouco discriminativas (Seção~\ref{sec:features_redundancia}). O Experimento~4 (14 \textit{features}) elimina amplitude, desvio padrão do fluxo bruto, máximo do Savitzky-Golay e as derivadas; o Experimento~5 (13 \textit{features}) remove adicionalmente \texttt{Feature\_Savgol\_Min}; e o Experimento~6 (12 \textit{features}) ablaciona a \texttt{kmeans\_centroid\_dist}. Em todos eles, o melhor F1-score permanece em torno de 0,99 --- ou seja, \ia{\textbf{a remoção dessas variáveis não degrada o desempenho}}. Os resultados completos desses três experimentos (tabelas e matrizes de confusão) estão no Apêndice~\ref{ap:experimentos}; a Tabela~\ref{tab:comparacao_f1} (Seção~\ref{sec:comparacao}) consolida os valores.

\ia{Um achado merece destaque por ser, à primeira vista, contraintuitivo.} A \texttt{kmeans\_centroid\_dist} --- distância entre os dois centróides obtidos pela aplicação de K-Means ($K=2$) ao fluxo suavizado --- figura consistentemente entre as variáveis \emph{mais importantes} nos modelos de árvore, chegando a concentrar boa parte da importância no XGBoost. Ainda assim, removê-la (Experimento~6) \ia{\textbf{não piora o desempenho}}: a capacidade preditiva é simplesmente redistribuída para variáveis colineares, sobretudo \texttt{Feature\_Savgol\_std} --- que captura o mesmo fenômeno físico, a dispersão do fluxo causada pelos dois patamares dentro e fora do \textit{dip}. \ia{A lição é importante: \textbf{a importância atribuída por um algoritmo de árvore não é sinônimo de insubstituibilidade}; uma variável pode concentrar importância apenas por oferecer um ponto de corte conveniente, enquanto outras compensam sua ausência.} Na prática, isso permite simplificar a \textit{pipeline} eliminando o passo de K-Means sem custo de desempenho. \ia{[Esta mesma conclusão é repetida em §\ref{sec:discussao} (Discussão), nas Conclusões (Cap.~6, achados e contribuições) e no Apêndice~\ref{ap:experimentos} --- consolidar em um único lugar.]}
...
O ajuste de limiar foi aplicado aos quatro modelos do Experimento~5 (13 \textit{features}, divisão 80\%/20\%; detalhado no Apêndice~\ref{ap:experimentos}). Para cada modelo, foram calculadas as métricas em função do limiar $\tau \in [0{,}01,\; 0{,}99]$. A Tabela~\ref{tab:threshold_comparacao} resume pontos operacionais representativos.
...
& $\tau = 0{,}01$ (máx. sens.) & 0,7921 & 1,0000 & 0,8840 & 0,9501 & 42 / 0 \\
\hline
Regressão Log. & $\tau = 0{,}16$ & 0,9938 & 1,0000 & 0,9969 & 0,9988 & 1 / 0 \\
\hline
XGBoost & $\tau = 0{,}30$ & 0,9875 & 0,9875 & 0,9875 & 0,9875 & 2 / 2 \\
\hline
CatBoost & $\tau = 0{,}37$ & 0,9875 & 0,9875 & 0,9875 & 0,9875 & 2 / 2 \\
\hline
\end{tabular}
\end{table}

\textbf{Destaque:} A Regressão Logística com $\tau = 0{,}16$ atinge sensibilidade de 100\% com apenas 1 falso positivo \ia{--- ponto operacional ideal para triagem em que minimizar falsos negativos é prioritário}.
...
Os dados de Quaoar foram \textbf{gentilmente cedidos pelo autor principal} de~\citet{Quaoar2023}, no qual a redução fotométrica completa do evento e a caracterização dos dois anéis Q1R e Q2R são apresentadas. Reforçando o que já se mencionou, a curva de Quaoar não figura no banco SQLite usado para treinar os modelos, garantindo que o teste é, de fato, sobre dados externos à distribuição de treino.
...
A Figura~\ref{fig:quaoar_curvas} apresenta a curva analisada após o pré-processamento, com o \textit{dip} central do corpo principal e as estruturas rasas dos anéis. A detecção da \textbf{curva inteira} como ocultação é trivial --- corpo central mais dois anéis fornecem um sinal forte --- e serve apenas como \textit{baseline} (primeira linha da Tabela~\ref{tab:quaoar_recortes}, $\hat{p}\approx0{,}995$ no CatBoost). O interesse real está em \textbf{sondar os anéis por recortes}, isolando cada assinatura física.
...
\begin{table}[htbp]
\centering
\caption{Probabilidade de ocultação $\hat{p}$ atribuída por CatBoost e XGBoost (Exp.~2) à curva inteira (\textit{baseline}) e a cada recorte da curva Gemini-'Alopeke \textit{Red}-$z$. Janelas em segundos relativos ao instante 2022-08-09 06:34:49,26 UTC.}
...
\hline
Curva inteira & corpo central $+$ 2 anéis (\textit{baseline}) & 0,9951 & 0,9996 \\
\hline
...
\ia{O estudo de caso reforça, na prática, quatro conclusões dos experimentos formais:}
...
\ia{Cinco observações principais emergem da análise conjunta:}
...
\begin{enumerate}
	...
	\item \textbf{Generalização em curvas reais.} O Experimento 3 (teste somente em curvas reais) mostra queda de $\approx$2~pp em F1, com AUC-ROC ainda acima de 0,995, indicando que as \textit{features} capturam sinal real \ia{e não apenas artefatos da distribuição sintética}.
...
A Figura~\ref{fig:feature_importance_enxuto} apresenta os gráficos de importância de variáveis para os quatro modelos no Experimento~2 (configuração enxuta, 11 \textit{features} úteis após a remoção das redundantes); concentramos a análise nesta configuração porque são justamente essas variáveis que permanecem em uso na \textit{pipeline} final. As figuras análogas das demais configurações estão no Apêndice~\ref{ap:experimentos}. A interpretação dos valores deve considerar as diferenças metodológicas discutidas na Seção~\ref{sec:feature_importance_metodologia}: cada modelo emprega uma métrica própria --- MDI normalizado para soma~1 (Random Forest), \textit{weight} normalizado (XGBoost), \textit{PredictionValuesChange} normalizado para soma~100 (CatBoost) e $|\beta_j|$ sem normalização (Regressão Logística) ---, de modo que as escalas horizontais não são diretamente comparáveis entre os painéis.

Nos modelos de árvore (Random Forest e XGBoost), as variáveis mais relevantes são consistentemente:

\begin{itemize}
	\item \texttt{Feature\_Savgol\_std}: a \textit{feature} dominante em ambos os modelos --- isolada, responde por $\approx$0,57 da importância no XGBoost. Mede a dispersão do fluxo suavizado, que cresce quando coexistem dois patamares (dentro e fora do \textit{dip}). Com a remoção de \texttt{kmeans\_centroid\_dist} (Experimento~2), ela absorve o papel discriminativo antes atribuído àquela variável colinear, como antecipado pelo estudo de ablação.
	\item \texttt{Max\_Drawdown}: captura diretamente a magnitude da maior queda acumulada de fluxo, assinatura primária de uma ocultação.
	\item \texttt{Occ\_depth} e \texttt{Occ\_duration\_s}: quantificam a profundidade e a duração do evento segundo critérios observacionais.
	\item \texttt{MinLogP\_Ttest} (e, em menor grau, \texttt{Occ\_chi2\_ratio} e \texttt{MinLogP\_KS}): os testes estatísticos entre quartis detectam a heterogeneidade no fluxo esperada em curvas com ocultação.
\end{itemize}

\begin{figure}[p]
\centering
\noindent\textbf{Experimento 2 (11 \textit{features} úteis).}\\[0.5em]
\begin{minipage}[t]{0.48\textwidth}
	\centering
	\includegraphics[width=\linewidth]{Tese/pngs/exp5/feature_importance_logistic_regression.png}
	\vspace{0.3em}
	\small (a) Regressão Log.
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
	\centering
	\includegraphics[width=\linewidth]{Tese/pngs/exp5/feature_importance_random_forest.png}
	\vspace{0.3em}
	\small (b) Random Forest
\end{minipage}\\[1em]
\begin{minipage}[t]{0.48\textwidth}
	\centering
	\includegraphics[width=\linewidth]{Tese/pngs/exp5/feature_importance_xgboost.png}
	\vspace{0.3em}
	\small (c) XGBoost
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
	\centering
	\includegraphics[width=\linewidth]{Tese/pngs/exp5/feature_importance_catboost.png}
	\vspace{0.3em}
	\small (d) CatBoost
\end{minipage}\\[0.5em]
\caption{Importância de variáveis por modelo --- Experimento 2 (configuração enxuta, 11 \textit{features}). (a) Regressão Logística, (b) Random Forest, (c) XGBoost, (d) CatBoost.}
\label{fig:feature_importance_enxuto}
\end{figure}

As figuras análogas das configurações com conjuntos maiores de \textit{features} (Experimentos 4 e 5), que permitem acompanhar como a importância relativa das variáveis se redistribui à medida que o conjunto é reduzido, estão reunidas no Apêndice~\ref{ap:experimentos}.

Naquelas configurações, a \textit{feature} \texttt{kmeans\_centroid\_dist} figura entre as mais importantes, e isso não é um artefato dos modelos: a análise exploratória dos dados confirma que ela é, por si só, discriminativa. A Figura~\ref{fig:histograma_kmeans} mostra que as curvas negativas se concentram em valores baixos de distância entre centróides --- coerente com um fluxo de regime único, em que o K-Means não encontra dois agrupamentos distintos ---, enquanto as curvas positivas se espalham por valores altos, refletindo a coexistência dos dois patamares (dentro e fora do \textit{dip}). É justamente por parecer tão promissora que o resultado do estudo de ablação (sua remoção não degrada o desempenho, pois \texttt{Feature\_Savgol\_std} é colinear e a substitui Seção~\ref{sec:resultados_exp4})  se torna um achado relevante\ia{, e não uma simplificação trivial}.

\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=0.85\linewidth]{Tese/pngs/resultado3/histograma_kmeans_por_classe.png}
\caption{Distribuição da \textit{feature} \texttt{kmeans\_centroid\_dist} por classe (1--99 percentil). As curvas negativas concentram-se em valores baixos (regime único) e as positivas em valores altos (dois patamares), evidenciando o poder discriminativo da variável.}
...
A Tabela~\ref{tab:cross_validation} apresenta a média e o desvio padrão das métricas na validação cruzada estratificada 5-fold (Experimento~5, 13 \textit{features}; detalhado no Apêndice~\ref{ap:experimentos}).
...
A Tabela~\ref{tab:mcnemar} apresenta os resultados do teste de McNemar entre pares de modelos (Experimento~4, 14 \textit{features}; detalhado no Apêndice~\ref{ap:experimentos}).
