...
\begin{enumerate}
	...
	\item \textbf{Análise de curvas de aprendizado:} evidência de saturação em suas duas dimensões, a do volume de treino (ablação 65/35, Apêndice~\ref{ap:experimentos}) e a da convergência do \textit{logloss} por iteração de \textit{boosting} (Seção~\ref{sec:curvas_aprendizado_exp}), orientando futuras decisões sobre coleta de dados.
...
\begin{enumerate}
	...
	\item \textbf{\textit{Split} único por experimento:} cada experimento utiliza uma única partição treino/teste. A implementação de validação cruzada estratificada (Seção~\ref{sec:cross_validation}) mitiga parcialmente essa limitação; uma validação cruzada repetida com múltiplas sementes fortaleceria as conclusões.
