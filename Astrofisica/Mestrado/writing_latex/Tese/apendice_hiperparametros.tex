...
Não foi utilizada busca em grade (Grid Search) nem validação cruzada para escolha desses hiperparâmetros; os valores acima correspondem a uma configuração \textit{baseline} fixa, mantida em todos os experimentos para permitir uma comparação justa entre eles, variando-se apenas o conjunto de \textit{features} e a composição do conjunto de teste. Para trabalhos futuros, pode-se explorar validação cruzada estratificada com otimização de F1-score ou AUC-ROC para refinar os hiperparâmetros.
