...
%%%%
\begin{abstract}
Ocultações estelares, eventos nos quais um corpo do Sistema Solar bloqueia temporariamente a luz de uma estrela distante para um dado observador, constituem uma das técnicas mais precisas para a caracterização física de asteroides, Centauros e objetos transnetunianos. A análise da curva de luz resultante permite inferir tamanho, forma projetada e, em casos favoráveis, atmosfera e anéis do corpo ocultante, com resolução espacial da ordem de quilômetros. O crescimento do volume de dados produzido por redes de telescópios e catálogos como o Gaia, no entanto, tornou a triagem manual de todas as curvas um problema relevante e motivou o desenvolvimento de métodos automatizados de detecção. Esta dissertação apresenta uma \textit{pipeline} completa para detecção automatizada de ocultações estelares em curvas de luz, baseada em técnicas de aprendizado de máquina. O problema é formulado como classificação binária: cada curva é descrita por um vetor de características numéricas - estatísticas do fluxo, suavização por Savitzky-Golay, \textit{max drawdown}, distância entre centróides do K-means, testes estatísticos entre quartis e métricas observacionais de profundidade, duração e razão sinal-ruído - e classificada como positiva (ocultação presente) ou negativa (ausência de evento). Quatro classificadores foram treinados e comparados: Regressão Logística, Random Forest, XGBoost e CatBoost. O conjunto de dados reúne 1693 curvas (802 positivas e 891 negativas, incluindo negativos artificiais obtidos por recorte manual e curvas sintéticas). Seis experimentos avaliaram o impacto da seleção de \textit{features} e a generalização para dados exclusivamente reais. Os resultados demonstram desempenho elevado e consistente: F1-score entre 0,978 e 0,994 e AUC-ROC superior a 0,998 no conjunto de teste. A análise de redundância reduziu o espaço de 28 para 13 características sem perda de desempenho. Um estudo de ablação mostrou que a \textit{feature} derivada do K-means, apesar de figurar entre as mais importantes nos modelos de árvore, é dispensável --- evidenciando que a importância atribuída por algoritmos não implica insubstituibilidade. A \textit{pipeline} permite ainda o ajuste do limiar de decisão em pós-processamento, operando em regime de alta sensibilidade sem retreinamento, no qual o custo de perder um evento genuíno (falso negativo) supera o de revisar um alarme falso (falso positivo). Como demonstração em dados reais externos ao treino, um estudo de caso sobre uma ocultação pelo objeto transnetuniano (50000) Quaoar aplicou a \textit{pipeline} a recortes da curva: embora os anéis finos recebam probabilidade baixa em termos absolutos, ela é cerca de duas ordens de grandeza superior à atribuída a trechos de ruído semelhantes, de modo que um simples ajuste de limiar recupera esses eventos sutis sem introduzir falsos alarmes --- ilustrando o uso da ferramenta para revisitar curvas complexas em busca de descobertas. O trabalho contribui, assim, com uma ferramenta reproduzível de triagem e priorização de curvas de luz para o fluxo de trabalho em ocultações estelares.
\end{abstract}

%%%%
\begin{foreignabstract}
Stellar occultations - events in which a Solar System body temporarily blocks the light of a distant star to a given observer - are among the most precise techniques for the physical characterization of asteroids, Centaurs and trans-Neptunian objects. Analysis of the resulting light curve yields the projected size, shape and, in favorable cases, atmosphere and rings of the occulting body, with spatial resolution on the kilometer scale. The growing data volume produced by telescope networks and catalogs such as Gaia has, however, rendered manual screening of all light curves a relevant problem, motivating the development of automated detection methods. This dissertation presents a complete pipeline for the automated detection of stellar occultations in light curves using machine learning techniques. The problem is cast as binary classification: each curve is described by a vector of numerical features - flux statistics, Savitzky-Golay smoothing, max drawdown, K-means centroid distance, statistical tests between quartiles, and observational metrics of depth, duration and signal-to-noise ratio - and classified as positive (occultation present) or negative (no event). Four classifiers were trained and compared: Logistic Regression, Random Forest, XGBoost and CatBoost. The dataset comprises 1693 light curves (802 positive and 891 negative, including artificial negatives obtained by manual cropping and synthetic curves). Six experiments assessed the impact of feature selection and generalization to exclusively real data. Results demonstrate consistently high performance: F1-scores between 0.978 and 0.994, and AUC-ROC above 0.998 on the test set. Redundancy analysis reduced the feature space from 28 to 13 variables with no loss in performance. An ablation study showed that the K-means-derived feature, despite ranking among the most important in tree-based models, is dispensable - demonstrating that algorithmic feature importance does not imply irreplaceability. The pipeline also supports post-hoc decision-threshold tuning, enabling high-recall operation without retraining - a strategy suited to scientific screening, where the cost of missing a genuine event (false negative) outweighs that of reviewing a false alarm (false positive). As a demonstration on real data external to training, a case study on a stellar occultation by the trans-Neptunian object (50000) Quaoar applied the pipeline to slices of the light curve: although the thin rings receive a low absolute probability, it is about two orders of magnitude higher than that assigned to similar noise segments, so that a simple threshold adjustment recovers these subtle events without introducing false alarms - illustrating the use of the tool to revisit complex curves in search of discoveries. This work thus contributes a reproducible screening and prioritization tool for the stellar-occultation workflow.
\end{foreignabstract}
