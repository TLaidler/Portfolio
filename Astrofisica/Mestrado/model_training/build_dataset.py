...
def build_and_save_dataset(positives, negatives_db, artificial_negatives, 
    ...
    cols_order = ['curve_name', 'source', 'occ',
                  ...
                  # Feature de K-Means no fluxo (1) - distância entre centroides
...
def plot_kmeans_hist_by_class(
    ...
    # Remove NaNs
    data = df[[feature, occ_col]].dropna()

    # Limites por percentil (globais)
    low = data[feature].quantile(pmin / 100)
    high = data[feature].quantile(pmax / 100)

    # Filtra outliers
    data_filt = data[(data[feature] >= low) & (data[feature] <= high)]

    pos = data_filt[data_filt[occ_col] == 1][feature]
    neg = data_filt[data_filt[occ_col] == 0][feature]
